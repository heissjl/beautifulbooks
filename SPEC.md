...
**Die Ausgaben unter einem gefalteten Cover** sind nach zwei Kriterien sortiert (`orderEditionsForMarket`), in dieser Reihenfolge: **erstens das Verdikt** — die Ausgabe, deren registriertes Bild dieses Cover ist, steht vorn (Julian, 2026-09-09: „die version die das gleiche aktuelle cover hat wie die isbn sollte zuerst vorgeschlagen werden, nicht nach jahr sortiert"; bei *Beloved* trägt ein gefaltetes Cover Vintage International 2025 und 2004, und es ist die 2004er ISBN, zu der der Verlag dieses Bild führt) —, **zweitens der Markt**: ISBN aus dem eigenen Sprachraum, dann irgendeine ISBN, dann das jüngste Jahr. Vorher war es die Ankunftsreihenfolge von Open Library, also das Alter des Datensatzes.
...
- `sitemap.xml` (Startseite, About, Impressum, Privacy, kuratierte Werke) und `robots.txt` (`/api/` gesperrt, weil jeder Aufruf dort eine externe Anfrage kostet; `/go/` gesperrt, weil jeder Aufruf dort als Klick zählt).
