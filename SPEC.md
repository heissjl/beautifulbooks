...
- **Startseite** `/`: Hero („Judge a book by its covers.“), Suchfeld, Sprache als Chips, kuratierte Wand; mit `?q=` die Trefferliste. **Rechts neben Überschrift und Suchfeld, ab `lg`, ein Rondell aus sieben Covern eines Buchs** (ROADMAP 1.9; seit 2026-09-10 als Fächer aus vier, seit 2026-09-11 als Rondell): das Produktversprechen als Bild, als Link auf die Wand. Es kreist in 40 s einmal von selbst. Unter dem Zeiger **folgt das Cover unter dem Zeiger der Maus**: vorne und hinten entgegengesetzt, und ein Kreis mit der Maus dreht den Ring ganz herum. Unter `prefers-reduced-motion` dreht es nicht von selbst, folgt aber der Maus. Die sieben Cover stehen fest (`lib/herofan.ts`, heute *Dune*) und sind aus dem gebauten Index nach Abstand gewählt. **Jedes Paar liegt im Hash über der lockersten Schwelle der Seite** (20 Bit, zwei Cover mit derselben ISBN; gemessen ≥ 26 Bit) und ≥ 0,39 in der Farbe; ein Test hält das. Die Zeile darunter nennt **Titel und Autor, keine Zahl** (§1). Unterhalb von `lg` wird es **gar nicht gerendert, auch nicht vom Server**, damit das Suchfeld auf dem Telefon nicht unter die Kante rutscht. Per CSS versteckt lud ein Telefon alle Cover eines Bildes, das es nie zeigte.
