...
**Eine Stelle, an der die Umsetzung hinter dieser Regel zurückbleibt**, gemessen und in der Roadmap eingetragen; bis sie behoben ist, beschreibt der Satz oben das Soll und nicht das Ist.

**Übersetzungen bleiben eigene Karten.** Regel 2 vergleicht Titel *und* Autor, und eine Übersetzung hat einen anderen Titel — die Regel greift also nie, obwohl der Satz oben genau das verlangt. Bei der Suche „ansichten böll" sind fünf von sechs Karten derselbe Roman (ROADMAP 6.15). Eine verlässliche maschinelle Verbindung zwischen den Datensätzen gibt es nicht; welche Regel dazukommt oder ob dieser Absatz umgeschrieben werden muss, entscheidet die Messung dort.

**Karte und Detailseite meinen dieselbe Gruppe** (seit 2026-09-11, ROADMAP 6.13). Open Library führt dasselbe Buch oft als mehrere Werk-Datensätze (*Ansichten eines Clowns*: sechs). Die Suche fasst sie nach Regel 2 zu einer Karte zusammen; die Detailseite sucht auf Seite 0 einmal gezielt nach Titel und Autoren-Key, wendet dieselbe Regel an und lädt die Ausgaben dieser **Geschwister** nach ihren eigenen — ohne Google-Anfrage, höchstens zwölf, die größten zuerst. Die Karte kennt dabei womöglich weniger Geschwister als die Detailseite, weil sie nur die ersten zwanzig Treffer der Suche sieht: die Wand nennt dann **mehr** Ausgaben als die Karte, nie weniger. Antwortet die Geschwistersuche nicht, lädt die Wand nur den einen Datensatz und behauptet nichts über andere. Die Jahrzehnte-Seiten (5.4a) zählen weiter nur den einen Datensatz, auf dem ihre Schwelle gemessen ist.
...
| `foreign` | Gruppe passt nicht — **die Mehrheit** | AbeBooks und eBay (DE: Booklooker), **zuerst nach der Nummer, dahinter nach Titel, Verlag und Jahr** (seit 2026-09-10, ROADMAP 1.11/1.11a; vorher nur die Wortsuche, weil antiquarische Angebote oft keine ISBN tragen — gemessen an einer mexikanischen *Steppenwolf*-Ausgabe lieferte die Nummer 11 Angebote, die Wörter 8). Ein Laden mit zwei Fragen sagt im Label, welche er stellt: „AbeBooks · ISBN“, „AbeBooks · title & year“ | „This printing's ISBN was registered in Turkey. Marketplaces that list copies from anywhere come first; no shop was asked." |
...
- **F1.4a Ein Suchfeld in der Kopfzeile**, auf jeder Seite außer der Suche selbst (`HeaderSearch`, ROADMAP 6.28). Die Kopfzeile ist `sticky`, es ist also auch aus der Mitte einer Wand von 300 Covern erreichbar und bleibt während der Ladeszene bedienbar. **Ein einziges Eingabefeld:** auf breiten Schirmen zeigt es CSS allein, auf dem Telefon öffnet eine Lupe dasselbe Element über der Kopfzeile — nie eine zweite Kopie mit derselben Beschriftung. **Keine Sprach-Pillen** (sie gehören zum großen Feld der Startseite), aber ein vorhandenes `?lang=` wird mitgenommen; die Zielseite zeigt die passende Pille, es wird also nichts unsichtbar angewandt. Der Wert wird beim Absenden aus `window.location` gelesen und **nicht** über `useSearchParams`, weil der Hook jede Seite mit diesem Feld aus dem statischen Rendern nähme.

  **Der Zurück-Link der Detailseite heißt deshalb nicht mehr „Search".** Neben einem Suchfeld trügen zwei Bedienelemente dasselbe Wort und täten Verschiedenes: der Link führt zur Trefferliste zurück, aus der man kam, das Feld fängt von vorn an. Er heißt jetzt **„Results"**, wenn ein `?q=` vorliegt, sonst **„Home"** — entschieden wird das an der Query, nicht an der Adresse, denn `?lang=de` allein ergibt ebenfalls eine Adresse ungleich `/` und führt trotzdem nicht zu Treffern.

- **F1.5** URL-Zustand `/?q=…&lang=…`; Back-Button und Teilen funktionieren. **Enter im Suchfeld sendet ab** — am 2026-09-10 von Julian am Gerät geprüft, nachdem zwei automatisierte Anläufe es nicht konnten: das Browser-Panel schickt Tastendrücke ohne Tastenwert. Das Feld sitzt in einem `<form>` mit `type="submit"`-Knopf, die implizite Übermittlung des Browsers trägt es also; wer das Formular umbaut, prüft es wieder von Hand (ROADMAP 0.8).
...
- **F1.6a Das Warten zeigt ein Mosaik** (ROADMAP 6.19a, seit 2026-09-09). Während die Suche läuft — 1 bis 13 s — und während die Jahrzehnte-Seite gebaut wird (4,5 bis 12,9 s, seit 2026-09-10), baut sich das Gesicht eines Autors aus den Covern seiner eigenen Bücher auf: eine Wand in falscher Reihenfolge, gedimmt, deren Kacheln in zufälliger Ordnung an ihren Platz finden, während sich die Dämpfung hebt. **Genau eine vorgerechnete JPEG-Datei** aus `public/loading` (83–102 KB auf dem Telefon, 124–159 KB am Rechner) und ein Manifest von 2,3 KB; **kein Cover wird einzeln geladen**, und keine der zwanzig Vorlagen kostet eine Anfrage an eine fremde Quelle. Die Vorlage wird **bei jedem Anzeigen neu gewürfelt** (seit 2026-09-10; die zuletzt gezeigte ist ausgeschlossen, damit der Wechsel sichtbar ist), die Datei beim **ersten Tastendruck** geholt, und das Feld bekommt seine Höhe aus dem Manifest, bevor das Bild da ist. **Ist der Aufbau vorbei, bevor die Seite da ist, atmet das fertige Bild weiter** (Deckkraft zwischen 0,7 und 1, 2,5 s je Zug) — das ist der Normalfall, nicht die Ausnahme: eine Suche läuft 1 bis 13 s, der Aufbau ist nach 3 s vorbei. `prefers-reduced-motion` bekommt dasselbe Atmen von Anfang an, **ohne Aufbau**. **Solange das Bild fehlt, steht die Überschrift über einer Fläche in seiner Größe, die im selben Atem pulsiert wie das fertige Bild — auch wenn es nie kommt** (seit 2026-09-11, ROADMAP 6.33 — die kleine pulsierende Cover-Wand, die vorher die Lücke füllte, gibt es nicht mehr: Julian, „der sollte nirgendwo mehr existieren"). Gezeigt wird das Mosaik bei der Suche, vor der Jahrzehnte-Seite und auf einer Werkseite, zu der keine Vorschau vorliegt (Aufruf von außen, Rückweg von der Jahrzehnte-Seite); wo eine Karte ein Cover mitgegeben hat, bleibt es beim Cover-Fächer aus F2.12. **Wonach gesucht wird, steht über dem Bild** — als Überschrift in der Display-Schrift, nicht als Auflage auf dem Mosaik: eine Zeile auf dem Bild braucht einen eigenen Grund und liest sich dann wie ein aufgeklebtes Etikett. Wer auf dem Bild zu sehen ist und aus wie vielen Covern es besteht, steht klein darunter, damit es niemand für ein Suchergebnis hält (N12).
...
- **F2.3 Zähler statt Versprechen.** Während des Ladens „N covers · M of K editions checked“, danach „N covers from K editions“, bei Kappung „first 1,500 of K editions checked“, bei Abbruch „…, the source stopped answering“. Ein 1-px-Balken zeigt den Fortschritt. Darunter der Hinweis, dass die meisten Datensätze keinen Scan tragen. K ist die Summe über den Datensatz und seine Geschwister (§2.1, seit 2026-09-11).
- **F2.4 Sprach-Tabs.** Cover gruppiert nach Sprache der Ausgaben, die sie tragen. Reihenfolge: die im Suchfilter gewählte Sprache zuerst, dann Englisch, dann Deutsch (`LEAD_LANGUAGES`), dann nach Häufigkeit, „Unknown“ zuletzt. Die Ladeszene wartet, bis die gewünschte Sprache (sonst Englisch) da ist, längstens bis 300 Ausgaben geprüft sind, damit die vorderen Reiter nicht unter dem Mauszeiger nachrücken. Unterhalb von `sm` stehen die Reiter in einer seitlich scrollbaren Zeile. **Am Ende der Reiter steht „All languages"** mit der ganzen Wand, neuester Druck zuerst (seit 2026-09-11, ROADMAP 6.8); die Reiter brechen auf jedem Gerät um und belegen **am Telefon höchstens zwei, am Desktop höchstens drei Zeilen**; was nicht passt, steht hinter „+n more" (N14) — nie aber „Unknown" und „All languages", unter „Unknown" steht oft ein großer Teil der Wand (Julian). Die Pille ist keine Gruppe: die Ladeszene wartet weiter auf die gewünschte Sprache.
...
- **F2.6a Die zweite Spalte ohne Auswahl: das Werk.** Solange nichts gewählt ist, zeigt die Seitenleiste auf breiten Bildschirmen `WorkPanel` statt einer Ausgabe — der einzige Ort der Seite, der vom **Buch** spricht statt von einer Ausgabe. Inhalt, alles aus geladenen Ausgaben gezählt, ohne eine einzige Anfrage: eine Zeile mit Jahresspanne und Zahl der Verlage (`editionSpan`, erst wenn mehr als eine Seite geladen ist, sonst behauptet sie eine Spanne und korrigiert sich); der Klappentext mit Nennung der Ausgabe, aus der er stammt; die Einladung, ein Cover zu wählen; ein Link auf den Open-Library-Datensatz mit dem Hinweis, dass er dort korrigiert werden kann. Der Klappentext ist **Verlagswerbung für eine Ausgabe**, nie eine Beschreibung des Werks, deshalb wird er zugeschrieben. `blurbFor` (lib/works.ts) wählt ihn nach Sprache vor Länge: bei *Wolf Hall* ist der längste von 26 Ausgaben portugiesisch. Weicht die Sprache von der gesuchten ab, wird sie genannt.  **Seit 2026-09-14 hat der Klappentext zwei Quellen** (ROADMAP 6.46): die Beschreibungen der Ausgaben, die fast ausschließlich von Google Books stammen (gemessen: 0 von 567 Open-Library-Ausgaben tragen eine), und die Beschreibung des **Werks** bei Open Library (132 von 134 veröffentlichten Werken tragen eine). Der Server holt den Werk-Datensatz auf Seite 0 **nur, wenn keine Ausgabe einen Klappentext hat** — Google aus, Kontingent leer oder nichts Passendes — und immer, wenn `BLURB_SOURCE=work` gesetzt ist; das ist der Schalter weg von Google. Das kostet eine Open-Library-Anfrage je kalter Seite 0, gecacht wie das Werk, und keine Google-Anfrage. `lib/blurb.ts` bereinigt Open Librarys Markdown (Verweise, Fußnoten, „Also contained in“); **ein Text, der Wikipedia nennt, wird Wikipedia zugeschrieben (CC BY-SA)**, jeder andere dem Open-Library-Datensatz — nie nur „via Open Library“, wenn der Text selbst eine andere Herkunft nennt (N12).
...
- **F2.9 Verdikt an den Kauf-Links** (`verifyIsbnCover`, `lib/works.ts`). Kein zweiter Schwellenwert: das Urteil benutzt die Faltung der Wand selbst, damit Seitenleiste und Wand sich nie widersprechen.
  ...
  **Nicht gefaltet heißt nur dann „anders", wenn gefaltet werden konnte** (seit 2026-09-11, ROADMAP 6.32). Fehlt dem gewählten Cover — samt der in es gefalteten Scans — oder Googles Bild die Signatur, weil ein Bild nicht geholt werden konnte, lautet das Urteil `uncompared`: das Bild steht neben dem Satz, dass nicht verglichen werden konnte, und die Reihe bleibt die des Normalfalls. Anlass: Rowohlt 2011, „a different cover" neben einem sichtbar gleichen Bild (N12).
  ...
  **In der Seitenleiste sprechen nur `differs` und `uncompared`**, beide mit dem Bild des Verlags daneben (Julian, 2026-09-11: „es sollte nur eine anmerkung geben bei differs“; `uncompared` kam am selben Tag mit ROADMAP 6.32 dazu und blieb beim Zusammenführen). `differs` ändert, was der Leser tun sollte — die Nummer liefert einen anderen Umschlag, also sucht die erste Reihe nach dem Bild; bei `uncompared` wägt der Leser das Bild selbst, und die Knöpfe bleiben die der ISBN. **Offen, Julian entscheidet:** ob `uncompared` so sprechen soll oder wie die übrigen schweigt (ROADMAP 6.38). Die übrigen Zustände standen als Satz unter jedem Druck und änderten nichts; Schweigen behauptet nichts (N12). **Die Wortlaute unten gelten weiter**: die About-Seite zitiert alle (F6), und `lib/verdicts.ts` bleibt ihre einzige Quelle.
...
- **F2.12 Ladeszene.** Karten geben Titel, Autor und Cover per sessionStorage mit (`useWorkPreview`); die Seite zeigt sofort Titel und Hero-Cover, setzt die ersten eintreffenden Cover als Fächer in Szene (`LoadingStage`, bis zu vier) und lässt sie per FLIP auf ihre Kacheln fliegen (`flyCovers`, respektiert `prefers-reduced-motion`). **Der Fächer beginnt mit dem Cover, das die Karte hingestellt hat** — es wird die erste Kachel, mit derselben Adresse und ohne neuen Einzug, statt von einem anderen Bild abgelöst zu werden (seit 2026-09-11). **Eine Kachel erscheint erst, wenn ihr eigenes Bild geladen ist**, und die Szene übergibt erst an die Wand, **wenn deren erste sechs Cover angekommen sind** — die, die die Wand in ihrer eigenen Reihenfolge zuerst zeigt, nicht die ersten sechs der Seite 0 —, längstens nach 8 s. Davor flog der Fächer auf eine Reihe, die zu zwei Dritteln leer war (ROADMAP 6.25a). **Bei warmem Cache endet die Szene sofort** — sind die Cover bekannt und Seite 0 schon gehasht, wenn die Szene beginnt, endet sie im selben Augenblick, statt zwei Cover einzufliegen (repariert 2026-09-10; davor stand auf dem Rückweg von der Jahrzehnte-Seite vier Sekunden ein Fächer über einer fertigen Wand). **Die Zeile, die sagt, worauf gewartet wird, steht über dem Bild und im Textfluss**, nicht als Auflage darauf (Julian, 2026-09-09; für `LoadingStage` und für das Mosaik aus F1.6a): über einem Bild liest sich ein Satz als dessen Unterschrift, und genau das darf er nicht sein — er sagt, worauf gewartet wird, nicht, was zu sehen ist (N12).
...
- **Startseite** `/`: Hero („Judge a book by its covers.“), Suchfeld, Sprache als Chips, kuratierte Wand; mit `?q=` die Trefferliste. **Rechts neben Überschrift und Suchfeld, ab `lg`, ein Rondell aus sieben Covern eines Buchs** (ROADMAP 1.9; seit 2026-09-10 als Fächer aus vier, seit 2026-09-11 als Rondell): das Produktversprechen als Bild, als Link auf die Wand. Es kreist in 40 s einmal von selbst. Unter dem Zeiger **folgt das Cover unter dem Zeiger der Maus**: vorne und hinten entgegengesetzt, und ein Kreis mit der Maus dreht den Ring ganz herum. Unter `prefers-reduced-motion` dreht es nicht von selbst, folgt aber der Maus. Welches Buch es zeigt, wird **bei jedem Besuch neu gezogen**, aus allen kuratierten Werken, für die der gebaute Index sieben passende Cover findet: heute 90 von 105. Die Ringe stehen in `data/hero-rings.json` und werden von `scripts/build-hero-rings.ts` beim Bauen gerechnet; der Browser bekommt nur Kennungen und Namen. **Jedes Paar eines Rings liegt im Hash über der lockersten Schwelle der Seite** (20 Bit, zwei Cover mit derselben ISBN) und in der Farbe über dem Vierfachen des „Looks like this“-Tors. Fehlt einem Cover die Farbangabe, zählt das Paar nicht als verschieden. Ein Test prüft jeden Ring. Die Zeile darunter nennt **Titel und Autor, keine Zahl** (§1). Unterhalb von `lg` wird es **gar nicht gerendert, auch nicht vom Server**, damit das Suchfeld auf dem Telefon nicht unter die Kante rutscht. Per CSS versteckt lud ein Telefon alle Cover eines Bildes, das es nie zeigte.
...
Zwei Cover, ein Klick für das, das man lieber ansieht. Titel und Autor stehen seit 2026-09-11 unter jedem Cover (Julian: „wir müssen noch titel und autor anzeigen"); die Zeile unter der Überschrift bittet trotzdem: „Judge the cover, not the book!" Vorher erschienen die Titel erst auf der Rangliste `/versus/board`. Hervorgegangen aus `lab/hotornot` (5.8, Spielart 4); die Rechnung liegt in `lib/hotornot/`, damit Lab-Simulation und Seite dieselbe benutzen.
...
- **F7.2 Vorrat.** Eingefroren in `data/versus-pool.json` (E18), gebaut von `scripts/build-versus-pool.ts`: 1000 Cover aus 239 Büchern (`mix-1000-paperwhite`, seit 2026-09-11; vorher `mix-200-paperwhite` und `mix-100-paperwhite`), bis zu fünf verschiedene Designs je Buch, reihum gezogen: jedes Buch sein erstes Cover, bevor eines sein zweites bekommt. Ein Vorrat wächst, statt neu gezogen zu werden (`keep`): die Cover des Vorrats, aus dem er hervorging, kommen zuerst, soweit sie die Regeln noch bestehen, und dessen Stimmen und Meldungen zählen auf der neuen Rangliste mit (`inherits`), für die Cover, die beide enthalten. Neue Stimmen gehen an den neuen Namen. Ausgelassen werden die Cover, ohne die Cover, die ein Mensch schon als Nicht-Cover erkannt hat (`excluded`, mit Grund). Eingefroren, weil ein neuer Index sonst die Cover unter bereits abgegebenen Stimmen austauschen würde. **Ein Buch bringt nie dieselbe Jacke zweimal** (`sameJacket`, seit 2026-09-12): gefaltet wird bei dHash ≤ 8, und darüber hinaus bis 20 Bit, wenn auch die Farbe nah genug ist — erlaubt sind 0,45 Farbabstand bei 10 Bit, linear fallend auf 0,20 bei 20. Struktur allein trennt zwei Scans einer Jacke nicht von zwei Gestaltungen (F7.5, Historie). **Ins Spiel kommt nur, was groß scharf bleibt und kein Titel auf weißem Papier ist** (Julian, 2026-09-11): das L-Bild von Open Library mindestens 450 px hoch und im Hochformat (Breite zu Höhe 0,5 bis 0,85), höchstens drei Viertel der Fläche Papierweiß, und kein klassisches Reclam-Heft (mindestens die Hälfte Reclam-Gelb bei Kontrast bis 20), gemessen vom Skript am L-Bild (`lib/hotornot/quality.ts`) und in `data/cover-measures.json` festgehalten; und nicht in einer der fünf Zonen von `looksPlain` (`lib/hotornot/pool.ts`), in denen Titelseiten, Titel auf Papier und Seiten-Scans auf vergilbtem Papier liegen. Alle Grenzen an Kontaktbögen gesetzt (Julian, 2026-09-11: „wir brauchen noch bessere regeln gegen weiße cover, seiten-scans und wir sollten klassische reclam-cover ausschließen"). Das ist eine Auswahl fürs Spiel, kein Urteil über das Cover: die Seite zeigt diese Cover weiter (N12, §9.3 Schritt 12), und ein Vorrat aus einem einzelnen Werk behält sie.
- **F7.3 Speicher.** Redis aus dem Vercel Marketplace, mit Zeitlimit wie jeder externe Aufruf. Die Variablen tragen das Präfix `STORAGE_`; erkannt werden Endung und Schema, nicht ein fester Name: ein REST-Paar (`…REST_API_URL`, `…REST_API_TOKEN`, nie der Read-only-Token) wird bevorzugt, sonst eine `STORAGE_…URL` mit einer `redis://`- oder `rediss://`-Adresse, angesprochen über das Paket `redis`. Die Preview hatte am 2026-09-11 nur `STORAGE_REDIS_URL`. Die Spiellogik liegt über fünf Befehlen (RPUSH, LRANGE, HGETALL, HSETNX, SET NX EX), gleich für beide Wege. Ohne Speicher spielt `next dev` aus dem Arbeitsspeicher; ein Produktions-Build antwortet 503 und nennt die gesuchten Namen, nie Werte. **Eine Stimme ist zwei Cover, der Sieger und ein Tag, und nichts über den Spieler** (N11). **Ein Klick liest nie alle Stimmen** (seit 2026-09-14): die Paarung führt einen laufenden Zwischenstand der Elo-Wertungen, fragt je Klick nur die Zahl der Stimmen (`LLEN`) und holt die seit dem letzten Stand neuen; alle 25 Stimmen liegt der Stand als Zeichenkette unter `versus:<pool>:tally`. Die volle Liste liest nur die Rangliste, und die wird je Instanz einmal pro Minute gezählt.
...
- **F7.7 Keine Wiederholung in kurzer Folge.** Der Browser schickt mit jeder Anfrage die zuletzt gesehenen 100 Cover mit (`seen`); das nächste Paar meidet sie, solange mindestens zwei andere übrig sind. Die Liste bleibt im Browser, der Server speichert nichts über den Spieler (N11). Vorher kam ein übersprungenes Cover sofort wieder (Überspringen ist kein Spiel, das Cover blieb das am wenigsten gesehene), und nach der Aufwärmphase begann jedes zweite Paar mit denselben zwanzig Covern an den Enden (Julian, 2026-09-11).
- **F7.8 Nach dem Pick.** Das gewählte Cover hebt sich mit einem Ring, das andere verblasst, dann gleitet das Paar hinaus und das nächste herein (Variante A, von Julian gewählt). **Die nächsten zwei Paare werden samt Bildern schon geholt, während das aktuelle angesehen wird** (nacheinander, jedes meidet die Cover des vorigen; seit Julians Wunsch, vorher eines); der Stempel dauert so seine 0,26 s und nicht die Ladezeit von archive.org. Der Bereich der Cover schneidet seitlich ab, damit ein hereingleitendes Paar die Seite nicht verbreitert. Unter dem Paar steht der letzte Pick mit Link auf die Buchseite mit genau diesem Cover (`/book/<Werk>/cover/<Cover>`), in einem neuen Tab: wer ein Cover schön genug findet, soll es kaufen können. Auch auf der Rangliste führen Cover und Titel dorthin. Unter jedem Cover des Paars ein kleiner „Share"-Knopf, der dieselbe Adresse teilt (`ShareMenu`: nur Links, keine fremden Skripte; Satz „Beautiful or ugly? …"). Die Rangliste sagt je Cover „won 8 of 12" und je Ende einen Satz („Too early to say", „In front, but not settled", „Settled"), nicht mehr Anteile an plausiblen Ranglisten. Je Ende fünf Cover; ein Knopf unter jeder Liste zeigt zwanzig (`?top=20`, `?flop=20`).
- **F7.9 Zwei Cover aus einem Buch.** In 15 % der Paare stammen beide Cover aus demselben Buch, wenn das Buch ein weiteres im Spiel hat (`SAME_BOOK_SHARE`) — „welcher *Gatsby* ist schöner", die Frage, die nur diese Seite stellen kann (Julian, 2026-09-12). Möglich ist das nur, weil der Vorrat vorher sicherstellt, dass ein Buch nie dieselbe Jacke zweimal enthält (F7.2).
...
- **N8 Bilder.** Cover laufen seit dem 2026-09-09 über die **eigene Route `/img/<S|M|L>/<ol-123|gb-abc>`** (ROADMAP 1.3), davor der CDN. Der Pfad trägt eine **Cover-ID, nie eine URL**: die Zieladresse wird mit `coverUrlFor` neu gebaut, genau wie `/go/[provider]/[isbn]` den Händler-Link aus der Tabelle baut — eine Bild-Weiterleitung, die eine URL aus der Anfrage nimmt, ist ein offener Proxy. `proxiedCoverSrc` schreibt nur Adressen um, die dieser Code selbst gebaut hat, und lässt alles andere direkt laufen; das ist die sichere Richtung.
  ...
  **Ein Fehlschlag wird nicht gecacht** (`no-store` auf 400 und 502): ein schweigendes archive.org ist eine Episode, keine Tatsache über das Cover — dieselbe Regel wie F1.7. **Und er sagt, warum** (ROADMAP 6.25, seit 2026-09-10): eine Zeile `bb.img` (`lib/coverlog.ts`) mit Cover-ID, Größe, Quelle, dem Status der Gegenseite oder dem Grund, dass es keinen gab (`timeout`, `not-image`, `error`), und der Dauer — nichts über den Leser; die 502 trägt denselben Wert als `X-Cover-Upstream`. Ein 429 oder 403 von dort hieße, dass Open Library *uns* drosselt, was seit 1.3 möglich ist, weil alle Cover von einer Adresse geholt werden.

  **Belegt in Produktion am 2026-09-10:** derselbe Abruf `x-vercel-cache: MISS` in 2,12 s, dann `HIT` in 0,24 s.
...
- **N10 Rate-Limit** (`lib/ratelimit.ts`, `app/api/rate.ts`): Token-Bucket pro IP und Route, im Speicher, ohne Abhängigkeit. `search` 30/20 pro Minute, `works` 120/60, `isbn` 40/20, `availability` 6/3, `img` 800/400 (seit 2026-09-10; 400/300 reichten für eine Wand von 300 Covern, aber nicht für sie plus die Mosaike einer Trefferliste, ROADMAP 6.25), dazu ein gemeinsamer Eimer `google` 20/5 für alle Anfragen, die ein Kontingent kosten können (Seite 0, ISBN-Nachschau; nie ein Mosaik). Antwort 429 mit `Retry-After`. **Es bremst den Stoß, nicht den Tag** (5 pro Minute sind 7.200 pro Tag) und zählt pro Instanz; es ist kein Sicherheitsmerkmal.
...
- **N13 Erklärtexte sagen, was zu sehen ist — nicht, wie es zustande kam.** (Julian, 2026-09-09, an der Fußzeile der Jahrzehnte-Seite: „die beschreibung ist zu wissenschaftlich. die nutzer interessieren sich nicht für die schwellen. maximal darf dort stehen, was zu sehen ist und woher es kommt und der rückverweis auf die wand.")

  In einem Erklärtext der Oberfläche stehen höchstens drei Dinge: **was auf dem Schirm ist, woher es kommt, und der Weg zurück.** Dazu, wo es nötig ist, der Satz, der eine Vollständigkeit verneint (N12) — der ist keine Erklärung, sondern eine Einschränkung der Aussage.

  **Nicht hinein gehören die Regeln der Maschine:** Schwellenwerte („mindestens 20 Cover über 4 Jahrzehnte"), Sortierordnungen, die man ohnehin sieht, Sonderfallbehandlungen („Datensätze ohne Jahr stehen am Ende"), und Beteuerungen über die eigene Arbeitsweise („gezählt, nicht geschätzt"). Sie sind richtig und sie sind belegt — sie stehen in dieser Spec, wo sie hingehören. Vor den Augen eines Lesers machen sie aus einer Bildunterschrift ein Methodenkapitel.

  **Der Unterschied zu N12 ist wichtig, damit nicht das eine gegen das andere ausgespielt wird:** N12 verbietet, mehr zu behaupten, als geprüft wurde. N13 verbietet, das Geprüfte auszubreiten. Ein Satz kann beides einhalten — „Covers from Open Library, each in the decade of the earliest printing that carries it. What the catalogues never scanned is missing here too." behauptet keine Vollständigkeit und erklärt keine Schwelle.

  **Ausgenommen ist die About-Seite**: sie ist der Ort, an dem die Arbeitsweise ausführlich stehen darf und soll, samt Verdikten, Quellen und Lücken (§10). Wer es wissen will, findet es dort; wer nur Cover ansieht, wird nicht damit aufgehalten.

- **N14 Telefon und Desktop sind gleichwertig.** (Julian, 2026-09-11: „wir müssen sicher gehen, in Zukunft das mobile und Desktop eine vergleichbare Experience anbieten.") Anlass war der [Testbericht vom 2026-09-11](docs/tests/2026-09-11-mobil.md): auf der Startwand waren auf dem Telefon 9 von 18 Titeln abgeschnitten, auf dem Desktop einer; der Platzhalter der Suche passte nicht ins Feld; ein Abstand, der auf dem Desktop Luft ist, war auf dem Telefon eine leere Fläche. Alle drei Maße waren für den Desktop gewählt und nie auf dem Telefon angesehen worden.

  Daraus drei Regeln:
  1. **Was auf dem Desktop ganz zu lesen ist, ist es auch auf dem Telefon.** Ein Titel, ein Autor, ein Platzhalter darf auf dem Telefon umbrechen oder kürzer formuliert sein, aber nicht öfter abgeschnitten werden als auf dem Desktop. Ein Titel, der nach dem ersten Wort abbricht („Der…"), ist kein gekürzter Titel, sondern keiner.
  2. **Jeder Zustand, den der Desktop hat, hat das Telefon auch** — Laden, Fehler, leer, gewählt — und umgekehrt. Eine Funktion, die auf dem Telefon fehlt (oder nur über einen Umweg erreichbar ist), braucht eine Entscheidung in §6, nicht einen Zufall im CSS. E13 (Wand vertikal, Seitenleiste als Schublade) ist eine solche Entscheidung.
  3. **Geprüft wird in zwei Breiten: 390 × 844 und 1280 × 800.** Eine Änderung an der Oberfläche ist erst fertig, wenn sie in beiden angesehen ist; wo sie Text, Abstände oder Kacheln betrifft, mit gemessenen Zahlen (abgeschnittene Zeilen, Breite des Platzhalters gegen die des Felds) statt nur mit einem Blick.
...
| E21 | 2026-09-11 | E-Book-ISBNs | **Die Seite handelt nur von gedruckten Büchern; eine E-Book-ISBN wird nie gezeigt, nie verlinkt und nie für ein Urteil verwendet** (Julian: „uns geht es nur um physische Bücher“). Anlass: Googles Bild des blauen *Infinite Jest* hing an der E-Book-ISBN 9780748130986 (Hachette UK 2011), unter der kein Antiquariat etwas findet; die Druckausgabe mit demselben Cover ist 9780349121086 ([Testbericht M10](docs/tests/2026-09-11-mobil.md)). Das **Bild** eines E-Book-Bands darf bleiben, wenn es dasselbe Cover zeigt wie ein Druck — es wird dann der Druck-ISBN zugeordnet oder steht ohne ISBN. Erkennbar ist ein E-Book in den aufgenommenen Daten an Open Librarys `physical_format` („Epub“, „ebook“) und an Googles `saleInfo.isEbook`; **Gemessen 2026-09-11: Googles Feld meint nicht den Band**, sondern ein verfügbares E-Book — 25 von 100 Bänden der Fixtures, alle Beispiele gedruckte Bücher (Reclam, 238 Seiten; Library of America, 751) —, es wird deshalb nicht benutzt. Erkannt wird ein E-Book an Open Librarys Format; dieselbe Nummer fällt auch bei einem Google-Band (ROADMAP 6.35). Eine Suche nach der Ausgabe nennt Verlag und Jahr nur aus einem gedruckten Open-Library-Datensatz. **Die ISBN selbst verrät das Format nicht** (Julian fragte am 2026-09-11): sie besteht aus Präfix, Sprachraum, Verlag, Titelnummer und Prüfziffer, und welche Nummer ein Verlag seinem E-Book gibt, steht nur in einem Katalog, der das Format führt. Deshalb bleibt ein E-Book unerkannt, das kein Katalog als solches führt — wie 9780748130986 (Open Library: ohne Format), dessen ISBN-Link weiter ins Leere führt. **Hörbücher fallen ebenso** (Julian, 2026-09-11: „die sollten wir ebenfalls wie eBooks behandeln") — Audio CD, Kassette, „preloaded digital audio player" und jedes andere Hörformat; in den aufgenommenen Fixtures 12 Ausgaben. |
