...
- **Startseite** `/`: Hero („Judge a book by its covers.“), Suchfeld, Sprache als Chips, kuratierte Wand; mit `?q=` die Trefferliste. **Rechts neben der Überschrift, ab `lg`, ein Fächer aus vier Covern eines Buchs** (ROADMAP 1.9, seit 2026-09-10): das Produktversprechen als Bild, in der Bildsprache der Ladeszene, als Link auf die Wand. Die vier Cover stehen fest (`lib/herofan.ts`, heute *Dune*) und sind aus dem gebauten Index nach Abstand gewählt — Farbe ≥ 0,53, Hash ≥ 26 Bit —, ein Test hält das; die Zeile darunter nennt **keine Zahl** (§1). Unterhalb von `lg` wird er nicht gerendert, damit das Suchfeld auf dem Telefon nicht unter die Kante rutscht.
