...
- **Julian's screenshots and screen recordings stay local** (Julian, 2026-09-11: „wir müssen die Screenshots auch nicht auf GitHub laden in Zukunft"). Put them in `docs/tests/` next to the report that cites them — images there are git-ignored — and write everything the report needs into the text, because the pictures will not be in the repository. The eleven committed on 2026-09-11 went up before this rule.
- **Production logs: `vercel logs --project beautifulbooks`** (CLI logged in since 2026-09-11; `--since`, `--query`, `--status-code`, `--json`, `--follow`). **Vercel Hobby keeps them about one hour** — a query older than that answers 400. So a production fault is read while it happens (`--follow`) or within the hour, never the next morning; `bb.img` from the night of 2026-09-10 was already gone. Two traps: `--json` repeats each request about twenty times (deduplicate by `id`), and `--limit` counts those raw lines, so 1,000 lines can be half a minute — page backwards with `--until` to cover a visit.
- **Look at every UI change at 390 × 844 and at 1280 × 800** (SPEC N14, Julian 2026-09-11). On 2026-09-11 the home page cut 9 of 18 titles on a phone and 1 on a desktop, and the search placeholder did not fit its field — every one of those sizes had been chosen on a desktop and never looked at on a phone. Where text, spacing or tiles change, measure (clipped lines, placeholder width against field width) rather than glance.
...
- **The kanban board is a view of ROADMAP.md, never a second list.** `npm run kanban` (scripts/kanban.ts) writes the git-ignored `docs/kanban.html` from three sources: the item lines, the mermaid dependency graph under „Abhängigkeiten", and the branches whose unmerged commit subjects name an item. Nobody drags a card — a card moves because the roadmap changed. Owner is the one field the roadmap carries only as prose, so the script guesses it from documented rules (`**Wer:**`, „Julian entscheidet", an exceptions table, then the phase); a wrong guess is fixed in the roadmap line, not in the guess.
