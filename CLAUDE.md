...
- **Production logs: `vercel logs --project beautifulbooks`** (CLI logged in since 2026-09-11; `--since`, `--query`, `--status-code`, `--json`, `--follow`). **Vercel Hobby keeps them about one hour** — a query older than that answers 400. So a production fault is read while it happens (`--follow`) or within the hour, never the next morning; `bb.img` from the night of 2026-09-10 was already gone.
