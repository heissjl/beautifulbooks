...
- Cover dedupe (E8 phase 2, done): `lib/imagehash.ts` (dHash + contrast, server only) and `lib/coverhash.ts` (fetch small images with a 4 s budget per page, Next cache 30 d) produce signatures; `foldDuplicateCovers` in `lib/works.ts` folds identical scans into one cover with `similarIds` and runs on the client (SPEC §9.3 step 11). Cover counts therefore fall on a second visit, when more images are cached and hashed. Pass `dedupeCovers: false` in tests that mock fetch with JSON.
