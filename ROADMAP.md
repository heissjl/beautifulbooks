...
- **64 Punkte offen, 44 erledigt** (gezählt am 2026-09-13; neun davon sind neu aus Recherche und Risikoregister: 6.39–6.46 und 2.7). Phase 1 ist bis auf Julians Stichprobe (1.8) und den Rondell-Rest (1.9a) leer, Phase 2 hat nur noch Julians Konten und die Abnahme. Der größte offene Block ist Phase 6, und dort zuerst die Fehler, die ein Leser sieht.
...
| 6.46 | tun | 2 h | Google ist die einzige Klappentext-Quelle; Open Librarys Werk-Datensatz hat bei 99 % der veröffentlichten Werke eine (gemessen 2026-09-13) — ein Feld, null Anfragen |
...
| [6 Qualität](#phase-6--qualität-jederzeit-dazwischen) | Fehler, Daten, Oberfläche, Startseite | 26 | 25 | teils 6.6 (Geld), teils 6.18 (Julian) |
...
- [ ] **6.46 Klappentexte auch aus dem Open-Library-Werk-Datensatz.** (Aus Julians Frage vom 2026-09-13, [Risikoregister](docs/risiken-2026-09-12.md).) Gemessen: 567 Open-Library-Ausgabendatensätze über fünf Werke, keiner mit `description`; Google liefert 6–20 je Werk — **Google ist heute die einzige Quelle für den Klappentext im `WorkPanel`**, und der Open-Library-Parser kennt das Feld nicht. Open Library führt Beschreibungen aber auf **Werk**-Ebene, und `/works/<id>.json` wird schon geholt: ein Feld in `parseWork` (String oder `{type: '/type/text', value}`), null zusätzliche Anfragen; `blurbFor` bevorzugt weiter die Sprache, nimmt aber den Werk-Text, wenn keine Ausgabe einen trägt, und nennt die Quelle („Description from Open Library"). **Gemessen am 2026-09-13** (139 Werke, 134 antworteten, 5 Fehler): **132 tragen eine Beschreibung, 99 %**, Median 631 Zeichen, 120 englisch, 3 deutsch; ohne nur *KAFF* und *Gravity's Rainbow*. 15 nennen eine Quelle (Wikipedia mit Markdown-Link, Verlag) — die Texte sind **Markdown** und zu bereinigen, und ein Text, der Wikipedia nennt, steht unter CC BY-SA: die Zuschreibung im Panel nennt dann Wikipedia, nicht nur Open Library. Damit ist Google als Klappentext-Quelle ersetzbar. Zwei Stunden, Claude; Messwerte im [Risikoregister](docs/risiken-2026-09-12.md).
