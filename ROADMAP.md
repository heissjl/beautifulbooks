...
| 4 | **6.25 leere Kacheln** — nach einem Tag Produktion die `bb.img`-Zeilen lesen | Claude, nach einem Deploy | eine halbe Stunde plus Messung | Das Protokoll ist gebaut; ob Open Library uns drosselt, sagt erst ein Tag Logs — und drei `/img`-Antworten mit 15,6 s bei 325 Byte aus 6.25a gehören als Erstes hinein |
...
| b | **`bb.img`-Zeilen vom 2026-09-10 abends lesen** — Vercel-CLI installieren (`npm i -g vercel`) oder den Vercel-MCP autorisieren, oder im Dashboard selbst | Julian, dann Claude | M7–M9 haben vermutlich eine Ursache: Vercel bekommt Bilder nicht, dann fehlen Kachel *und* Faltung |
| c | **6.32** fehlende Signatur ergibt nie „anderes Cover" | Claude | M8, bricht §9.2 |
| d | **6.31** zweiter Versuch einer Kachel, **6.33** die alte Wand weg | Claude | M7, M11 |
| e | **6.30** Telefon wie Desktop (N14), Startseite und Messskript | Claude | M1–M3 |
| f | **6.34** Karten ohne Dubletten, Größe M | Claude | M4, M5 |
| g | **6.35** E-Books fallen weg (E21) | Claude, nach einer Messung | M10 |
...
- [ ] **6.34 Die Trefferkarten zeigen keine Dubletten und laden kleine Bilder.** ([Testbericht M4, M5](docs/tests/2026-09-11-mobil.md).) Der Kartenweg (`?summary=1`, `coverImages`) prüft nur Verlag und Jahr, nie das Bild; bei „David Foster Wallace" zeigen **3 von 8 Karten ein Paar, das die Wand selbst falten würde** (dHash 0, 2 und 8). Und jede Kachel lädt Größe **L**, obwohl sie ein Viertel der Karte füllt: 505 KB statt 195 KB für 14 Cover.
