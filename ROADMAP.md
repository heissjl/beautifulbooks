...
| **Experimente neben der Seite** | [lab/README.md](lab/README.md) — Kuratieren, Duell, Hot or Not, Mosaik, Ladebilder, Faltung, Palette |
...
- **Neu am 2026-09-11: 5.8a, Hot or Not online — die Preview spielt mit dem Speicher** (`/versus` auf dem Branch `worktree-session-2026-09-11`, SPEC F7). Freunde kommen über einen Freigabe-Link des Vercel-Connectors hinein (23 Stunden, an ein Deployment gebunden, nach jedem Push neu); die Previews bleiben geschützt. Schreibtest bestanden (26 Stimmen, 3 aussortiert). Offen: kaputte Bilder nur für den einen Spieler überspringen statt für alle, E21, der Satz in der Datenschutzerklärung, dann der Weg in die Produktion.
- **54 Punkte offen, 33 erledigt.** Phase 1 ist bis auf Julians Stichprobe (1.8) leer, Phase 2 hat nur noch Julians Konten und die Abnahme. Der größte offene Block ist Phase 6, und dort zuerst die Fehler, die ein Leser sieht.
...
- [ ] **0.14 Datenbank statt gebauter JSON-Dateien — wann, und welche.** (Julian, 2026-09-13: „Ist json dafür die beste Option oder sollten wir langsam über eine Datenbank nachdenken".) **Heute lautet die Antwort: noch nicht**, und das ist eine Messung, keine Meinung. Stand 2026-09-13: `data/cover-index.json` 1,5 MB (20.526 Cover, fest importiert in `lib/coverindex.ts`, also in jeder Serverfunktion), `versus-pool.json` 124 KB, `cover-measures.json` 219 KB (nur das Bau-Skript liest sie), `curated.json` 36 KB, `index-works.json` 37 KB, `decade-pages.json` 17 KB. **Eine Datenbank läuft bereits**: Redis hält die Stimmen des Spiels — das Einzige, was die laufende Seite schreibt (F7.3). Die Frage betrifft also nur die Lesedaten, und für die gilt E18: gebaut, nur lesbar, im Repo. Das kostet keine Laufzeit-Abhängigkeit, kann nicht ausfallen (N12: eine stumme Quelle ist kein leeres Ergebnis), ist im Diff prüfbar und aus den Skripten reproduzierbar.

  **Umsteigen, sobald eines davon eintritt** (dann in dieser Reihenfolge prüfen, nicht vorher bauen):
  1. **Der Index sprengt die Funktion.** Grobe Marke: über ~5 MB oder eine messbar langsamere Kaltstartzeit. Bei rund 500 Werken (5.1) wären es etwa 4–7 MB. **Billiger als eine Datenbank ist dann zuerst:** den Index je Werk zerlegen und nur laden, was die Anfrage braucht, oder ihn aus dem Repo in Vercel Blob auslagern und beim Build ziehen.
  2. **Die Seite muss zur Laufzeit etwas anderes schreiben als Stimmen** — ein Formular, ein Konto, eine Sammlung, eingereichte Cover. Dann ist es ohnehin eine Datenbankfrage, und 0.12 (Auftragsverarbeitung) wird gleichzeitig fällig.
  3. **Eine Abfrage, die ein Scan im Arbeitsspeicher nicht beantwortet:** blättern, filtern und sortieren über alle Cover, oder Auswertungen über die Zeit (5.8b, 3.1).
  4. **Zwei Schreiber brauchen Konsistenz** oder Daten müssen ein Deployment überleben, das sie nicht mitbringt.

  **Wenn es soweit ist:** Neon Postgres über den Vercel Marketplace (derselbe Weg wie der Redis-Speicher), Redis bleibt für die Stimmen — anhängende Liste und Hash, wofür es gebaut ist. Dorthin gehörten dann Stimmenverlauf und Rangliste (5.8b) und die Zahlen aus 3.1, nicht der Cover-Index.

  **Zwei kleine Dinge unabhängig davon:** `decade-pages.json` wird von einer Client-Komponente importiert und geht damit in den Browser (17 KB, dieselbe Falle wie `lib/coverindex.ts`, CLAUDE.md); und das Repo liegt in iCloud, wo jede neu gebaute 1,5-MB-Datei synchronisiert wird und iCloud gelegentlich Doppelkopien anlegt (am 2026-09-11 und 12 in `.next/types` gesehen). Beides spricht nicht für eine Datenbank, aber dafür, gebaute Dateien klein und geteilt zu halten.
...
- [ ] **5.8 Das Kuratieren als Minispiel — Reichweite aus dem Werkzeug.** (Julian, 2026-09-08: „die Kuratier-App ist auch eine Idee für eine Reichweiten-Aktion. Vielleicht kann man daraus ein Minispiel bauen … zB kuratieren, um dann Vorschläge zu bekommen. Oder Vergleich mit einem Freund (beide klicken auf den gleichen generierten Link, bekommen die gleiche Auswahl und am Ende einen Vergleich). Lass technische Einwände erstmal außen vor.") **Das Werkzeug aus 6.18 ist schon das Spiel** — ein Cover nach dem anderen, klicken, weiter; wer es einmal aufmacht, hört nicht nach fünf Büchern auf. Was fehlt, ist ein Grund, es zu teilen.
  ...
  **Spielart 4, seit dem 2026-09-11 als Prototyp: Hot or Not.** (Julian: „Ich will das hot or not spiel mit covern nachbauen. so wie damals facebook gestartet ist als zuckerberg noch an harvard war. wir wollen viral gehen können auf booktok indem wir das hässlichste und das schönste buchcover finden.") In [`lab/hotornot/`](lab/hotornot/README.md): zwei Cover, ein Klick, Elo zum Paaren, Bradley–Terry für die Rangliste, und ein Brett, das eine Krone erst vergibt, wenn sie ein Befund ist. **Die Frage ist nicht das Bauen, sondern wann die Seite „das hässlichste Cover" sagen darf.** Simuliert: wer beim ersten 90-%-Urteil postet, liegt in 13 % der Fälle falsch, weil das Brett nach jeder Runde neu gefragt wird; erst eine Krone, die drei Runden in Folge hält, drückt das auf 6 %. Für den Standardvorrat von 100 Covern heißt das bei mittlerer Einigkeit rund 2.900 Stimmen, also etwa hundert Mitspieler, bis „das hässlichste" belegt ist; die 230 Gestaltungen von *1984* brauchen mehrere Tausend. Im Standardvorrat steckt ein Platzhalter („This is not the actual book cover"), deshalb der Knopf „ist kein Cover" und vor jedem Posten ein menschlicher Blick auf die letzten fünf. **Offen:** wie einig sich echte Menschen sind (spielbar, nicht simulierbar); ein geteilter Online-Link braucht denselben Speicher wie Spielart 2; vor jedem Posten die Rechtefrage aus 5.5 und die Regel aus 5.6. Messung in der [Historie](docs/history.md).
...
- [ ] **5.8a Hot or Not online: eine Preview mit Speicher, die sich schnell in die Produktion ziehen lässt.** (Julian, 2026-09-11: „ja, bau die teilbare Seite für Freunde" — „aber erstmal nur im dev" — „kann man option 2 dann so vorbereiten, dass man das einigermaßen schnell in die produktion zieht wenn es klappt?") Folgt aus 5.8, Spielart 4. **Der Artifact-Weg fällt aus**, geprüft bevor etwas gebaut wurde: eine Claude-Seite mit geteiltem Speicher ist auf die claude.ai-Organisation des Besitzers beschränkt, Freunde von außen kommen nicht hinein ([lab/hotornot](lab/hotornot/README.md), „Offen").

  **So gebaut, dass der Schritt in die Produktion klein bleibt:**
  1. **Als Teil der Seite, nicht als Kopie.** `rating.ts` und `pool.ts` wandern mit ihren Tests von `lab/hotornot/` nach `lib/`, damit Brett und Simulation denselben Code behalten. Dazu eine Seite (Name offen), eine API für das nächste Paar, eine für die Stimme und eine für die Rangliste.
  2. **Ein Schalter** über eine Umgebungsvariable, nach dem Muster von E20: aus heißt 404. Zuerst nur in der Preview-Umgebung gesetzt.
  3. **Der Speicher hinter einer kleinen Schnittstelle**: online Upstash Redis aus dem Vercel Marketplace (eine Liste von Stimmen je Vorrat), in Tests ein Speicher im Arbeitsspeicher (N7). Eine Stimme enthält zwei Cover, den Sieger und einen Tag, **keine Kennung** (N11).
  4. **Gegen Missbrauch:** jede Stimme braucht ein vom Server ausgegebenes, signiertes Paar, dazu ein eigener Rate-Limit-Topf.
  5. **Bilder über `/img`** (1.3). Die Cover verlassen unsere Seite nicht, die Rechtefrage aus 5.5 (fremde Plattformen) stellt sich für das Spiel selbst also nicht.

  **In die Produktion ziehen heißt dann:** den Speicher auch an die Production-Umgebung hängen, den Schalter setzen, den Branch nach `main` mergen. Dazu ein Satz in der Datenschutzerklärung (Stimmen ohne Kennung) und eine Entscheidung E21, die E6 für genau diesen Speicher aufhebt.

  **Julian, vorher:** (a) im Vercel-Dashboard des Projekts einen Upstash-Redis-Speicher aus dem Marketplace anlegen und **nur mit der Umgebung Preview** verbinden; (b) ~~Deployment Protection prüfen~~ — *geprüft 2026-09-11 über den Vercel-Connector: Vercel Authentication ist an für alle Deployments außer den eigenen Domains, Preview-Links verlangen also eine Vercel-Anmeldung. Nicht abschalten: der Connector erzeugt für eine einzelne Preview einen befristeten Freigabe-Link (`get_access_to_vercel_url`), und den bekommen die Freunde. Den Speicher anlegen kann der Connector dagegen nicht — das geht nur im Dashboard, mit der CLI (`vercel install`) oder über die REST-API mit einem Token;* (c) die **Namen** der Variablen nennen, die der Speicher anlegt, nicht die Werte. **Claude, danach:** gegen genau diese Namen bauen, lokal mit dem Speicher im Arbeitsspeicher testen, und den Branch erst nach Julians Freigabe pushen, weil ein Push ein Preview-Deployment auslöst.

  **Julians Entscheidungen:** E21 (der erste Speicher, in den die laufende Seite schreibt; E6, E18), und ob das Spiel in Produktion verlinkt wird oder nur über den Link erreichbar ist.

  **Stand 2026-09-11: lokal fertig** ([SPEC F7](SPEC.md), [Historie](docs/history.md)). Die Spielseite `/versus` und die Rangliste `/versus/board`, drei APIs, signierte Paare, die Speicher-Schnittstelle und der eingefrorene Vorrat `data/versus-pool.json` stehen. Die ganze Suite (483 Tests) und der Build laufen, und das Spiel ist im Browser lokal durchgespielt. Julian hat das Präfix `STORAGE` gewählt; welche Endung Upstash dahinter setzt, erkennt der Code. **Es fehlen, in dieser Reihenfolge:** der Upstash-Speicher an der Preview-Umgebung (Julian), dann der Push des Branches (erst nach Julians Freigabe, weil er ein Preview-Deployment auslöst), dann der befristete Freigabe-Link für die Freunde über den Vercel-Connector.

  **Stand 2026-09-11, später am Tag: die Preview spielt mit dem Speicher.** Julians Speicher lieferte nur `STORAGE_REDIS_URL` und kein REST-Paar; der Code spricht jetzt beides (Paket `redis`). Auf der Preview antwortet `/api/versus/pair` mit `"store":"redis"`, und die Rangliste liest aus dem Speicher ([Historie](docs/history.md)). Der Freigabe-Link kommt über den Connector, gilt 23 Stunden und hängt am Deployment: **nach jedem Push ein neuer Link**, und solange gespielt wird, keine reinen Doku-Pushes auf den Branch. **Schreibtest bestanden:** Julian hat abgestimmt, die Rangliste zeigte 26 Stimmen auf 97 Covern und 3 aussortierte. **Offen:**
  1. **Ein Bild, das nicht lädt, nimmt das Cover heute für alle und dauerhaft aus dem Spiel.** Ein kurzer Aussetzer von archive.org würde es so für immer entfernen. Stattdessen: nur für diesen Spieler überspringen, global entfernt erst die Meldung eines Menschen, und die Rangliste zeigt die aussortierten Cover mit Grund. Beim Bauen bedenken: ein Push macht den Freigabe-Link ungültig.
  2. E21, dann der Satz in der Datenschutzerklärung, dann der Weg in die Produktion.

  **Stand 2026-09-11, abends: ein strengerer und doppelt so großer Vorrat.** (Julian, nach den ersten Stimmen: „wir müssen die cover ein bisschen stärker eingrenzen in sachen qualität. vllt nur diejenigen, die so groß angezeigt werden können ohne unscharf zu werden. und ein größerer abstand zu fast-weißen covern, die vor allem nur ein titel auf weißem hintergrund sind. und dann den korpus nochmal um 100 weitere bücher erweitern") Gebaut ([SPEC F7.2](SPEC.md), [Historie](docs/history.md)): nur Cover, deren L-Bild mindestens 450 px hoch ist und im Hochformat steht; vier helle Zonen, in denen fast nur Titelseiten und Titel auf Papier lagen, spielen nicht mit; der Index wächst um 100 Werke auf 239 (Julians 38 Picks aus `data/curated.json`, die das Auswahlskript bisher gar nicht las, und 62 neue Kandidaten, zuerst was auf BookTok gelesen wird), der Vorrat von 100 auf 200 Bücher. Online ging er zuerst mit den 139 Büchern des alten Index, weil Julian ihn gleich verschicken wollte; die übrigen folgen mit dem fertigen Index, und weil ein Vorrat unter gleichem Namen wächst statt neu gemischt zu werden, bleiben die Stimmen dabei gültig. Der Index ist fertig (239 Werke, 20.526 Cover), der Vorrat hat 200 Bücher, vier Nicht-Cover vom Kontaktbogen stehen mit Grund in `EXCLUDED`; **auf der Preview seit Commit `c469c18`** (200 Cover, geprüft über einen neuen Freigabe-Link; der vom ersten Push gilt nicht mehr). **Der neue Vorrat heißt `mix-200-paperwhite`**: die 26 Stimmen auf dem alten bleiben im Speicher unter dessen Namen, die Rangliste beginnt neu. Offen:
  1. **Ein Übergang nach jedem Pick** (Julian, 2026-09-11: „mache einen vorschlag für eine übergangsanimation nach jedem pick"). Drei Varianten als klickbare Skizze im Chat gezeigt: **A Stempel** (≈ 0,55 s; das gewählte Cover hebt sich mit Ring, das andere verblasst, das nächste Paar gleitet herein), **B Enthüllung** (≈ 1,2 s; beide Cover drehen sich um und zeigen Titel und Autor), **C Wisch-Stapel** (≈ 0,6 s; das Verlierer-Cover fliegt zur Seite, das gewählte landet in einem Stapel der eigenen Picks). **Empfehlung A**: bremst das Abstimmen nicht, und in der halben Sekunde lädt das nächste Paar vor — heute blitzt nach jedem Klick ein leerer Platzhalter auf. Den Stapel aus C als teilbares Bild „mein Geschmack" für später (5.8). Mit `prefers-reduced-motion` nur ein kurzes Überblenden. **Julian hat A gewählt** (2026-09-11: „Nimm Variante A (Stempel)"), Claude baut.

  **Stand 2026-09-11, nachts: nach der ersten Runde mit Freunden** ([SPEC F7.2, F7.7, F7.8](SPEC.md), [Historie](docs/history.md)). Julian nach 35 Stimmen der Freunde: bessere Regeln gegen weiße Cover, Seiten-Scans und Reclam; ein Weg zur Buchseite, um ein schönes Cover zu kaufen; keine Wiederholung in kurzer Folge; die Animation auf dem Telefon; Titel und Autor unter dem Cover; eine verständliche Rangliste; „mach dann eine version mit 1000 covers". Alles gebaut: Weiß- und Reclam-Regel am großen Bild und eine Regel für vergilbte Seiten, an Kontaktbögen gesetzt; die letzten 100 gesehenen Cover gesperrt; Vorladen des nächsten Paars (auf dem Telefon stand der Stempel vorher 1,9 s still) und kein seitlicher Überstand mehr; Titel und Autor unter jedem Cover, „Judge the cover, not the book!" darüber; nach dem Pick ein Link auf die Buchseite mit genau diesem Cover, auch von der Rangliste; die Rangliste sagt „won 8 of 12" statt Anteilen an Ranglisten; der Vorrat `mix-1000-paperwhite` mit bis zu fünf Covern je Buch, der Stimmen und Meldungen des 200er-Vorrats erbt. Offen:
  Dazu, auf Julians Wunsch („füge noch buttons hinzu, dass man die top 20 und flop 20 sich anzeigen lassen kann"): unter jeder Liste der Rangliste ein Knopf „Show top 20" bzw. „Show bottom 20" (`?top=20`, `?flop=20`, teilbar). Unter jedem Cover ein kleiner „Share"-Knopf für die Buchseite mit genau diesem Cover (Julian: „falls man eines lustig hässlich findet"). Die nächsten zwei Paare liegen immer schon vorgeladen bereit (Julian: „die nächsten 2 paare sollen auch immer schon vorgeladen werden"). **Auf der Preview seit Commit `96f954d`**: 999 Cover, 51 geerbte Stimmen, geprüft über einen neuen Freigabe-Link. Und von Hand ausgeschlossen, nach Durchsicht aller 1000 Cover: zwei Sorten erzeugter Platzhalter („COVER COMING SOON", „Note: This is not the actual book cover") und leere einfarbige Pappen.
  **Stand 2026-09-12: zwei Cover aus einem Buch, und erst der Beweis dafür.** (Julian: „es sollten ab und zu zwei cover vom gleichen buch kommen. aber hier ist wichtig vorher nochmal zu checken, dass es wirklich nicht zwei verschiedene scans vom gleichen cover sind.") Gemessen über die 1629 Paare aus je einem Buch und an Kontaktbögen angesehen: Struktur allein trennt zwei Scans einer Jacke nicht von zwei Gestaltungen. Neue Regel `sameJacket` als Keil — 0,45 Farbabstand bei 10 Bit, fallend auf 0,20 bei 20 Bit —, der Vorrat neu gebaut, und `nextPair` zieht 15 % der Paare aus einem Buch ([SPEC F7.2, F7.9](SPEC.md), [Historie](docs/history.md)).

  0. **Ein Ladebildschirm?** (Julian, 2026-09-11: „vielleicht brauchen wir noch einen ladebildschirm? was denkst du") Claudes Einschätzung: kein eigener Bildschirm. Gewartet wird nur noch beim ersten Paar, ein bis drei Sekunden mit zwei pulsierenden Kacheln; danach ist jedes Paar vorgeladen. Vorschlag: eine Zeile in den Kacheln („Shuffling 1000 covers…"), oder das atmende Mosaik der Suche (6.19a) wiederverwenden. **Julian entscheidet.**
  ...
  **Zwei Punkte, die vorher eine Entscheidung verdienen:**
  - ~~**Die Leseart skaliert nicht.**~~ **Erledigt 2026-09-14** (Julian: „ok, setze das um"): die Paarung führt einen laufenden Zwischenstand und liest je Klick nur die Zahl der Stimmen und die neuen; die Rangliste liest alle, einmal pro Minute. Gemessen bei 50.000 Stimmen: **50.100 gelesene Stimmen je Klick vorher, 1 jetzt** ([Historie](docs/history.md), SPEC F7.3).
  ...
  - **Sichtbarkeit, überdacht am 2026-09-13** (Julian: „warum nicht indizieren? das ist doch gut für reichweite"). Der Einwand stimmt, aber die beiden Seiten stehen verschieden da. **`/versus` trägt serverseitig keinen Satz** — die Seite rendert nur die Spielkomponente, das Paar holt der Browser; für einen Crawler ist das eine leere Seite, die sich bei jedem Abruf ändert (`force-dynamic`). **`/versus/board` ist das Gegenteil**: Titel, Autoren, Cover, Urteile und Links auf die Buchseiten, also lesbarer Inhalt. Deshalb in dieser Reihenfolge:
    1. **Die Rangliste indexieren**, sobald die Stimmen sie tragen. Dafür: `force-dynamic` durch eine kurze Zwischenspeicherung ersetzen (30–60 s — sonst liest jeder Crawler-Besuch die ganze Stimmenliste, dieselbe Stelle wie oben), zwei, drei feste Sätze darüber, `robots` auf `index`, Eintrag in die Sitemap, dazu ein OG-Bild fürs Teilen.
    2. **Die Spielseite erst indexieren, wenn sie Text trägt**: eine Überschrift, zwei Sätze, was das Spiel ist, die Zahl der Cover, Links auf die Rangliste und ein paar Bücher. Dann kann auch die Hülle statisch werden, das Paar kommt ohnehin aus dem Browser.
    3. **Die redaktionelle Liste aus 5.8b** („Die schönsten Cover, von Lesern gewählt"; je Buch „die Cover von *Dune*, sortiert") ist die eigentliche Reichweite: stabile Adressen, echter Text, Links in die Buchseiten.
    **Die Schwelle ist nicht ein Datum, sondern die Datenlage** (N12): Solange die Rangliste „Too early to say" sagt, verspricht ein Suchergebnis „die schönsten Cover" etwas, das die Stimmen nicht hergeben. Erst indexieren, wenn die Kronenregel etwas sagt. Danach ist F7.6 zu ändern.
