...
| Frage | Dokument |
|---|---|
| **Was die Seite ist und sein soll** | [SPEC.md](SPEC.md) — [§1 Produkt](SPEC.md#1-produktidee) · [§2 Domänenmodell](SPEC.md#2-domänenmodell) (Work, Edition, Cover, Kauf-Links, Index) · [§3 Funktionen F1–F6](SPEC.md#3-funktionale-anforderungen) · [§4 Regeln N1–N13](SPEC.md#4-nicht-funktionale-anforderungen) · [§5 Gestaltung](SPEC.md#5-gestaltung) · [§6 Entscheidungen E1–E21](SPEC.md#6-entscheidungen) · [§7 Gemessene Grenzen](SPEC.md#7-gemessene-grenzen-stand-2026-09-07) · [§8 Nummern-Konkordanz](SPEC.md#8-nummern-konkordanz) |
...
| **Was jeder offene Punkt wert ist** | [Bewertung der offenen Punkte](#bewertung-der-offenen-punkte-2026-09-12): Urteil, Aufwand und Grund je Punkt; das Brett liest die Tabelle |
...
- **64 Punkte offen, 45 erledigt** (gezählt am 2026-09-14, dazu 6.47 aus dem Firewall-Zweig; acht der offenen sind neu aus Recherche und Risikoregister: 6.39–6.45 und 2.7). Phase 1 ist bis auf Julians Stichprobe (1.8) und den Rondell-Rest (1.9a) leer, Phase 2 hat nur noch Julians Konten und die Abnahme. Der größte offene Block ist Phase 6, und dort zuerst die Fehler, die ein Leser sieht.
...
| 2 | **2.5 Search Console und Bing**, dazu **0.2** zweiter Google-Schlüssel, **0.13** Kontingent-Alarm und **2.4** Firewall auf *Log* ([Plan](docs/plans/PLAN-2.4-firewall.md)) | Julian | 45 Minuten | Jeder Tag ohne Sitemap ist ein verlorener Tag; ein Schlüssel für Arbeit *und* Betrieb verbraucht das Kontingent der Besucher |
...
| 4 | **6.5** Kleinigkeiten: Tippfehler-Toleranz, Label „about this book“, ein Weg aus der Telefon-Schublade zurück zur Wand, `priority` auf den ersten Kacheln (Mosaik-Ausfall und 6.8 erledigt 2026-09-11) | Claude | je etwa eine Stunde | ohne Abhängigkeit; `priority` vorher neu messen, 1.3 hat es verschoben |
| 5 | **6.18 Kuratierung zu Ende**, danach **6.17 Rotation** und das Jahr für **6.16** | Julian, dann Claude | ein Abend, dann eine Sitzung | Die Startseite ist das Erste, was ein Besucher sieht |
| 6 | **6.15 Schritt 3**, die Stichprobe zu Übersetzungen | Claude | eine Sitzung | 6.13 ist seit dem 2026-09-11 erledigt: die Wand lädt, was die Karte zusammenfasst. Offen ist, ob etwas Datensätze mit **anderem** Titel zusammenführen darf |
| 7 | **3.1 Analyse-Seite** | Claude | zwei Tage | Erst sinnvoll mit einer Woche echter Besucher |
| 8 | **Phase 5**, beginnend mit 5.3 an der fertigen Gattung 5.4a | beide | Wochen | Die eigentliche Reichweite |

**Aus dem [Testbericht vom 2026-09-11](docs/tests/2026-09-11-mobil.md)** (Telefon gegen Produktion, elf Befunde) sind zehn abgearbeitet — 6.25, 6.25a, 6.30, 6.31, 6.32, 6.33, 6.34, 6.35 und das Mitlesen von `bb.img` — und stehen als erledigte Punkte in Phase 6. **Offen aus dem Bericht ist einer:** **6.36**, gleicher Entwurf bei verschiedenen Verlagen (M9), und den entscheidet Julian. Aus 6.30 ist außerdem **6.30a** geworden: dieselbe Messung über Suche, Werkseite und Jahrzehnte-Seite.
...
Julian: „gehe nochmal durch die offenen Punkte und bewerte sie in ihrer Sinnhaftigkeit." Maßstab war dreierlei: das Produkt aus SPEC §1 (Cover nebeneinander sehen, die Ausgabe finden, nichts behaupten), die gemessenen Grenzen (Kontingent, Katalog-Latenz, kein Speicher) und der Engpass aus dem Stand oben — **niemand kennt die Seite**. Ein Punkt ist sinnvoll, wenn er einen Leser trifft, den es gibt oder bald gibt; er ist es nicht, wenn er auf einen Besucher, ein Konto oder eine Entscheidung wartet, die noch niemand getroffen hat.

Die Urteile sind ein festes Vokabular, weil das [Brett](kanban.html) sie liest: **tun** (sinnvoll, so wie er steht), **entscheiden** (nur Julian kann weiter), **zusammenlegen** (ist ein Teil eines anderen Punkts), **zurückstellen** (richtig, aber sein Auslöser fehlt noch — bleibt mit Nummer stehen, verlässt die aktiven Spalten des Bretts), **streichen** (Empfehlung; Julian entscheidet), **erledigt?** (sieht fertig aus und ist nur nicht abgehakt), **aufteilen** (ein lebender und ein toter Teil). Zusammenlegen, Streichen und die Fragezeichen sind Vorschläge, keine Änderungen — die Nummern bleiben, bis Julian es sagt.

**Die Bilanz:** von 55 offenen Punkten (vor den sechs neuen aus der [Recherche](docs/best-practices-2026-09-12.md), 6.39–6.44) sind **22 zu tun**, 4 warten auf eine Entscheidung, **22 sind zurückzustellen**, weil ihr Auslöser fehlt, und 7 sind Zusammenlegungen, Aufteilungen oder Fragezeichen. Der aktive Vorrat ist also halb so groß, wie die Zahl 55 behauptet — und die vier wertvollsten Minuten des ganzen Dokuments sind 2.5, 0.2, 0.5 und 0.13, alle bei Julian.
...
| 0.3 | tun | 10 min | Der einzige unerprobte Weg über 1.000 am Tag, und Voraussetzung für 0.13 |
| 0.4 | erledigt? | — | 2.3 ist gebaut „aus `IMPRINT_*`", die Seite ist online. Stehen dort Name und ladungsfähige Anschrift, ist der Punkt erledigt und nur nicht abgehakt; stehen dort Platzhalter, ist er dringend (§ 18 MStV) |
| 0.5 | tun | 20 min + DNS | Der Engpass laut Stand: ein Name, den niemand behält. Mit 2.2 zusammenlegen — kaufen und verbinden sind ein Vorgang |
| 0.7 | zurückstellen | — | Seit 1.1 geht es nur noch um die Titelsuche auf Seite 0; ohne Zahlen aus 3.2 nicht entscheidbar. Auslöser: 3.2 |
| 0.8a | tun | 2 min | Julian an einer echten Tastatur — oder Playwright MCP, das echte Tastendrücke schickt ([Recherche](best-practices-2026-09-12.md)); das Browser-Panel kann es nicht |
| 0.10 | zurückstellen | — | Sagt selbst „jetzt nicht entscheiden". Auslöser: 6.6 und Phase 3. In der aktiven Liste stiftet der Punkt nur Unruhe |
| 0.12 | zurückstellen | — | Entschieden, Restrisiko tragen; drei benannte Auslöser (Shop-Variante, Eingaben vom Leser, Anfrage einer Behörde) |
| 0.13 | tun | 10 min | Die Codeseite ist fertig; der Alarm in Cloud Monitoring ist der einzige Zähler, der zählt, was bei Google ankommt. Nach 0.3 |
| 2.2 | zusammenlegen | — | mit 0.5: `NEXT_PUBLIC_SITE_URL` umstellen und die Sitemap neu einreichen ist der zweite Schritt desselben Vorgangs |
| 2.4 | tun | 1 h | Ohne Überwachung merkt niemand einen Ausfall. Reihenfolge: erst Attack Mode prüfen (der Entwurf liegt auf `claude/firewall-2.4`), dann UptimeRobot — sonst meldet er Ausfälle, die es nicht gibt |
| 2.5 | tun | 20 min | Der höchste Wert je Minute im ganzen Dokument: ohne Sitemap in der Search Console existiert die Seite für Google nicht |
| 2.6 | tun | 10 min | Zwei Reste (OG-Bild in einem Messenger, Google-Verbrauch ablesen), dann abhaken |
| 1.8 | tun | 10 min | Kann Claude inzwischen selbst: Headless-Chrome rendert, was dem Skript fehlte — so entstanden am 2026-09-12 die Screenshots des Bretts |
| 1.9a | tun | 30 min + Julians Blick | Der Bau steht; vor dem Merge fehlen Julians Zahl und das Aufräumen der Schalter |
| 6.26 | tun | 1 h Messung | Julians eigene Bedingung („muss perfekt sein"). Erst messen, kuratiert gegen nicht kuratiert — seit 6.25 und 1.3 könnte die Klage erledigt sein; dann schließen |
| 6.5 | aufteilen | je 1 h | Die Verlaufskante ist seit 6.8 gegenstandslos (die Reiter brechen um, nichts scrollt seitwärts) — streichen. Tippfehler-Toleranz und der Weg aus der Schublade zurück treffen echte Leser — behalten. `priority` erst neu messen |
| 6.3 | tun | ½ Tag | 20 Sekunden Bühne treffen genau die deutschen Leser, die Julians Publikum sind. Die dritte Variante — sagen, worauf gewartet wird — ist die billigste und die ehrlichste |
| 6.2 | tun | ½ Tag | Ein russischer Titel auf der Karte für `crime and punishment` ist ein sichtbarer Fehler; die Lösung braucht keinen Eingriff ins Ranking |
| 6.36 | entscheiden | Julians Blick | Produktfrage: Dublette oder zwei Cover? Hängt mit 6.4 und 6.23 zusammen — vor beiden entscheiden |
| 6.15 | tun | 1 Sitzung | Schritt 3 ist eine Messung mit zwei guten Ausgängen: eine dritte Identitätsregel, oder §2.1 wird ehrlich umgeschrieben |
| 6.16 | aufteilen | 1 h / — | Schritt 2 (ein Jahr-Feld in der Kuratier-App) ist billig und macht die kuratierten hundert richtig — tun. Schritt 3 (Wikidata) ist ein neuer externer Aufrufer (E10) — zurückstellen |
| 6.6 | tun, kleiner | 1 Tag statt 2 | Blockiert 6.7, 6.4 und 6.23. Erst die kostenlosen Kandidaten (Hardcover, LibraryThing, DNB) messen; ISBNdb nur, wenn die drei am Schmidt-Testfall scheitern — der Punkt sagt selbst, dass ISBNdb dort vermutlich die falsche Antwort ist |
| 6.7 | aufteilen | 1 h | Schritt 2 (Imprint-Familien in `samePublisher`) hängt nicht an 6.6 und repariert den gemessenen Holt-Fall — jetzt. Schritte 1, 3 und 4 warten auf 6.6 |
| 6.4 | zurückstellen | — | Die Kennzeichnung ist die Antwort, wenn 6.36 „getrennt lassen" ergibt; entscheidet Julian „falten", entfällt der Punkt. Auslöser: 6.36 |
| 6.23 | zurückstellen | ein Wochenende (Stufe b) | Echtes Problem, aber hinter 6.6 und 6.36; wenn dran, nur die Stufen (a) und (b) — ein vortrainiertes Embedding vor jedem eigenen Netz |
| 6.12 | zurückstellen | ½ Tag | Für die kuratierten Werke durch den Index erledigt, lokal 100 % Signaturen beim ersten Aufruf gemessen. Auslöser: eine kalte Produktionsmessung, die ungehashte Cover zeigt |
| 6.11 | aufteilen | 5 min / Entscheidung | Die Recherche ist fertig — als Punkt erledigt. Übrig: ein Goodreads-Link neben WorldCat (tun, fünf Minuten) und ob „8.491 Leser bei Open Library" auf die Karte darf — eine §1-Frage, Julian |
| 6.30a | tun | 2 h | N14 ist Regel; die Startseite ist gemessen, drei Seiten nicht. Der Vorbefund (Kopfzeile bricht bei 320 px) zeigt, dass es Funde gibt |
| 6.27 | zusammenlegen | — | mit 0.1: der einzige Rest ist der Verfügbarkeits-Absatz. Fällt 0.1, fällt er mit |
| 6.9 | zurückstellen | — | Braucht den Index, den 5.4b auch braucht; zusammen bauen, wenn Phase 5 dran ist |
| 6.38 | entscheiden | eine Minute | Claude bereitet beide Varianten als Screenshot vor; dann ist es ein Blick |
| 6.17 | tun | 2 h | Rotation stündlich über ISR (Weg a) ist billig und erfüllt Julians Bedingung „vorgeladen"; die Liste liegt bereit |
| 3.1 | zurückstellen | 2 Tage | Richtig und wichtig — aber auf `localhost` misst man sich selbst. Auslöser: eine Woche echter Besucher nach 2.5 |
| 3.2 | zurückstellen | 5 min am Tag | Sobald es Besucher gibt; entscheidet 0.7. Auslöser: 2.5 |
| 3.3 | zurückstellen | — | Setzt Partnerprogramme voraus (Phase 4) |
| 5.1 | tun | ½ Tag | Der Weg über Katalogpopularität braucht keinen Besucher; 139 → 500 Adressen sind 500 Chancen zu ranken. Sichtbar wird das erst mit 2.5 |
| 5.2 | zurückstellen | — | Sagt selbst „nicht auf Verdacht". Auslöser: Search Console zeigt, dass Cover nicht ankommen |
| 5.3 | zurückstellen | — | Die Fabrik lohnt, wenn 5.4a Besucher zeigt (5.7) |
| 5.4 | aufteilen | 1 h (Fehler) | In (a) steckt ein offener **Fehler**: ein unvollständiger Lauf wird 24 h gecacht — ein Ausfall als Befund (N12). Gehört als eigener kleiner Punkt vor alles Übrige in 5.4. (b) braucht Julians Reihenliste, (c) ist billig und automatisch, (d) nach Phase 4, (e) zuletzt |
| 5.5 | zurückstellen | — | Rechtefrage zuerst (Julian); Mosaik und Clip liegen fertig im Lab |
...
| 4.4 | zurückstellen | — | Erst mit der Absicht auf Einnahmen |
| 4.5 | zurückstellen | — | Bedingt („falls"). Auslöser: 0.3 und 3.2 |
| 4.7 | zurückstellen | — | Fünfstellige Impressionen sind weit |
| 4.8 | zurückstellen | — | Drei Monate nach 4.7 |
| 6.39 | tun | 1 h | Vier „jedes Mal“-Regeln wurden trotz CLAUDE.md gebrochen; ein Hook kann nicht vergessen werden. Der übertragbarste Befund der Recherche |
| 6.40 | tun | 1 h | Das Abhaken hat fünf Schritte, und drei Sitzungen haben Teile ausgelassen — ein Befehl statt einer Erinnerung |
| 6.41 | entscheiden | 5 min + 1 h | Julian installiert die Erweiterung; damit fallen 0.8a, 1.8 und 6.38 auf einmal |
...
1. **Sechs Punkte tragen ihren Auslöser schon im Text** (0.10, 0.12, 5.2, 4.5, 6.12, 4.8) und stehen trotzdem in der aktiven Liste. Sie bleiben mit Nummer, wo sie sind; das Brett stellt sie ab jetzt in eine eigene Spalte, damit sie nicht als „offen" zählen, ohne dass jemand sie tun könnte.
2. **Ein Fehler versteckt sich in einem Feature-Punkt:** die Jahrzehnte-Seite cacht einen unvollständigen Lauf 24 Stunden (in 5.4a als „Offen" notiert). Das ist derselbe Riss wie 1.4 und 5.4a selbst — ein Ausfall, der wie ein Befund aussieht — und gehört nach 6.A, nicht in Phase 5.
3. **Was Claude jetzt allein tun kann**, ohne Julian und ohne Besucher, in Reihenfolge des Werts: 5.4a-Fehler, 6.7 Schritt 2, 6.30a, 6.2, 6.3, 6.16 Schritt 2, 6.17, 1.8, 5.1, 6.5 (die zwei lebenden Teile), 6.26 (Messung), 6.15 Schritt 3, 6.6 (kostenlose Kandidaten). Das ist eine Woche Arbeit, und keiner dieser Punkte wartet auf etwas.
...
flowchart LR
  subgraph Julian
    ...
    J636(["6.36 gleicher Entwurf, andere Verlage"])
    J638(["6.38 spricht uncompared?"])
    J19a(["1.9a die 90 Ringe ansehen"])
    J58a(["5.8a Hot or Not: veröffentlichen?"])
  end
  R626["6.26 kuratierte Liste fühlt sich perfekt an"]
...
| [6 Qualität](#phase-6--qualität-jederzeit-dazwischen) | Fehler, Daten, Oberfläche, Startseite | 26 | 26 | teils 6.6 (Geld), teils 6.18 (Julian) |
...
- [ ] **2.4 Betrieb.** UptimeRobot (kostenlos) auf `/api/search?q=1984`; Fehler vorerst über die Vercel-Logs, Sentry erst bei Bedarf. **Firewall-Vorschlag vom 2026-09-11 in [PLAN-2.4-firewall.md](docs/plans/PLAN-2.4-firewall.md)** (Julian: „mache einen Vorschlag, welche Firewall-Einstellungen wir machen müssen“). Kurzfassung: Attack Mode **aus lassen** (gemessen 2026-09-11: ein einzelner `curl` auf `/about` kommt mit 200 ohne Challenge, er ist also aus); Bot Protection und AI Bots je auf **Log**, eine Woche beobachten, AI Bots sperren oder nicht ist Julians Produktentscheidung; der einzige Rate-Limit-Platz des Hobby-Plans als Regel `api-google-burst` auf `/api/isbn/` und `/api/works/` ohne `summary=1`, 300 Anfragen je 10 Minuten und IP, erst Log, dann 429 — **sie bremst einen Stoß, schützt aber nicht den Tag**; zwei der drei Regelplätze frei für den Ernstfall. **Die 403 vom 2026-09-09 kam von der automatischen DDoS-Abwehr nach wiederholten Abrufen**, nicht von einer Projekteinstellung; eine Ausnahme für den Monitor gibt es auf Hobby nicht (System-Bypass erst ab Pro), sie ist aber vermutlich auch nicht nötig: `uptime-robot` steht in Vercels Verzeichnis verifizierter Bots. Die Sorge, Attack Mode träfe „jeden Crawler“, war zu groß: verifizierte Bots und Suchmaschinen kommen durch — nicht aber die Link-Vorschauen von WhatsApp, Telegram, Discord und Slack, die im Verzeichnis fehlen. **Stand 2026-09-14:** Mit der Vercel-CLI abgelesen: keine eigenen Regeln, Attack Mode aus, Bot Protection aus, AI Bots *Allow*. Die Regel `api-google-burst` liegt **als unveröffentlichter Entwurf** bei Vercel, nur protokollierend. Sie nimmt jetzt auch `sibling=1` aus, weil Geschwisterseiten seit 6.13 nie Google fragen. **Nächster Schritt: Julian**, 10 Minuten (Plan §5):
  - den Entwurf veröffentlichen, im Dashboard oder mit `vercel firewall publish --project beautifulbooks --yes`;
  - Bot Protection und AI Bots im Dashboard auf *Log* stellen, was die CLI nicht kann;
  - UptimeRobot einrichten — **später** (Julian, 2026-09-14: „das mit uptime robot mache ich später"). Die Firewall-Schritte hängen nicht daran.

  Danach eine Woche Log, dann Claude. Den Traffic liest auf Hobby nur das Dashboard, die CLI braucht dafür Observability Plus.

- [ ] **2.5 Search Console und Bing Webmaster Tools ab Tag 1**, Sitemap einreichen. *Dazu (Recherche 2026-09-12): den Core-Web-Vitals-Bericht der Search Console lesen — die ersten Felddaten von echten Geräten, die diese Seite je hatte; Laborwerte zählen nicht ([Best Practices](docs/best-practices-2026-09-12.md) A1).*
...
- [ ] **1.9a Der Rest des Rondells: Julians Blick auf die Ringe, und was vor dem Merge fehlt.** *Abgetrennt am 2026-09-12 von 1.9, wo beides als eingerückte Kästchen in einem **abgehakten** Punkt stand und deshalb in keiner Zählung vorkam.* Zwei Dinge: **(1) Julian sieht die 90 Ringe einmal an** — gewählt ist nach Bildabstand, nicht nach Auge, und zwei Ringe enthalten ein fast weißes Cover (*La Divina Commedia*). **(2) Vor dem Merge** des Rondells: Julians Wahl der Zahl, den `?hero=`/`?ring=`-Schalter aus `app/page.tsx` entfernen, Fächer- und Startzustand aufräumen. Der Bau selbst steht (`components/HeroRondell.tsx`, gemessen auf dem Dev-Server des Worktrees); der volle Text bleibt unter 1.9 und im [Archiv](docs/roadmap-archive.md#19).
...
- [x] **1.9 Der leere Platz oben rechts auf der Startseite.** Erledigt 2026-09-10 als Fächer aus vier Covern von *Dune*; **am 2026-09-11 zum Rondell aus sieben umgebaut** (Julian: „wie wäre es mit einem Rondell?“ und „7 ist aber eine gute Zahl“).
  ...
  - **Bildunterschrift:** Titel und Autor des gezeigten Buchs, gekürzt wie auf der Startwand (`tileTitle`), **keine Zahl** (N12).
  ...
  - **Welches Buch:** Seit dem 2026-09-11 **ist es bei jedem Besuch ein anderes** (Julian: „es sollte wechseln zwischen werken aus der kuratierten liste, die mehr als 7 cover über der entsprechenden schwelle haben“; auf Nachfrage „nur pro Besuch“). Kandidaten sind alle kuratierten Werke, für die der Index sieben Cover findet. Jedes Paar liegt **über der lockersten Hash-Schwelle der Seite** (20 Bit) und unterscheidet sich klar in der Farbe. Das trifft heute auf **90 von 105** zu. Die Ringe rechnet `scripts/build-hero-rings.ts` beim Bauen aus (`data/hero-rings.json`). Ein Test prüft jeden Ring und dass die Datei zum Index passt.
  - [ ] **Offen, Julian: die 90 Ringe einmal ansehen.** Gewählt ist nach Abstand, nicht nach Auge. Zwei Ringe enthalten ein fast weißes Cover, *La Divina Commedia* und *Архипелаг ГУЛАГ* (Sättigung 0), weil die Blässe-Regel nur das untere Viertel des jeweiligen Werks aussortiert. Eine feste Untergrenze wäre ein neuer Schwellwert, und der gehört durch Hinsehen gesetzt (6.10).
...
- [ ] **6.5 Kleinigkeiten aus dem Durchklick.** *Der Mosaik-Ausfall ist erledigt am 2026-09-11: die Mosaik-Anfrage einer Karte fragt nach 5xx oder Netzfehler nach 1,5 s ein zweites Mal (`components/useCardCovers.ts`), ein 404 oder 429 nicht; nachgestellt mit 503 auf jeder ersten Anfrage, alle zwölf Karten von „gravity’s rainbow“ fragten zweimal und bekamen ihr Mosaik ([Historie](docs/history.md#2026-09-11--eine-karte-fragt-zweimal-bevor-sie-sich-mit-einem-cover-begnügt-roadmap-65)). Offen bleiben die Kleinigkeiten dahinter.* *Dazu am 2026-09-08 beim Prüfen von 6.1 im Browser gesehen: bei `alice in wonderland` antwortete eine der Mosaik-Anfragen (`/api/works/<id>?summary=1`) mit **503**, und die Karte blieb leer, ohne dass irgendwo stand, warum. Das ist derselbe Riss wie 1.4, eine Ebene tiefer: der Ausfall einer Quelle sieht aus wie ein Buch ohne Cover. Die Wiederholung aus 1.10 sitzt nur im Suchpfad. Entweder wiederholt die Mosaik-Anfrage einmal, oder die Kachel sagt, dass sie nicht geladen werden konnte. **Am 2026-09-09 im Code bestätigt:** `components/useCardCovers.ts` fängt jeden Fehler in ein leeres `catch` („never surface it") und kennt keinen zweiten Versuch — die Wiederholung aus 1.10 sitzt allein in `searchWorks`.* Tippfehler-Toleranz (`gatsbee` liefert null Treffer ohne Vorschlag; ein Abgleich gegen die kuratierten Titel und die letzten Suchen wäre billig). Ein sichtbares Label „about this book“ auf Karten mit Sekundärliteratur, statt sie nur nach hinten zu rechnen. Eine Verlaufskante an der seitlich scrollbaren Reiterzeile auf dem Telefon. Ein Weg von der Telefon-Schublade zurück zur Wand, ohne zu schließen, zu scrollen und neu zu tippen. `priority` auf den ersten Kacheln, die Konsole meldet auf jeder Seite LCP-Warnungen. *Recherche 2026-09-12: **höchstens ein `priority` je Seite**, auf dem einen LCP-Kandidaten — mehrere erzeugen je ein Preload, das um Bandbreite konkurriert („priority abuse“); erst mit Felddaten aus der Search Console (2.5) entscheiden ([Best Practices](docs/best-practices-2026-09-12.md) A1).*
...
- [ ] **6.30a Dieselbe Messung über Suche, Werkseite und Jahrzehnte-Seite (N14).** *Vorbefund 2026-09-11: auf der Werkseite bei 320 px bricht der Seitentitel „Beautiful Books“ in der Kopfzeile auf zwei Zeilen um, sobald „Share“ daneben steht. Die Überbreite der Werkseite (806 px auf 390) ist behoben, gemessen von 320 bis 768 px ([Historie](docs/history.md#2026-09-11--die-messleiste-der-sprach-pillen-machte-die-seite-am-telefon-806-px-breit-roadmap-68-n14)); beim Messen im Browser-Panel vorher einen Frame erzwingen.* Die Startseite ist nur die, die Julian fotografiert hat. Messen bei 390 und 1280 px: abgeschnittene Zeilen, Platzhalter gegen Feld, leere Flächen, Zustände, die nur ein Gerät hat. Die Messung aus 6.30 (ein Stück JavaScript im Browser, im [Historie-Eintrag](docs/history.md#2026-09-11--vier-befunde-vom-telefon-behoben-roadmap-630-bis-633) festgehalten) als wiederverwendbares Werkzeug. Zwei Stunden, Claude.
...
- [ ] **6.27 Die übrigen Erklärtexte auf N13 durchgehen.** (Aus Julians Sprachregelung vom 2026-09-09, jetzt SPEC **N13**: in einem Erklärtext steht höchstens, was zu sehen ist, woher es kommt und der Weg zurück — keine Schwellen, keine Sortierordnungen, keine Beteuerungen über die eigene Arbeitsweise.)
  ...
  *Stand 2026-09-11 abends: die zwei Verdikt-Zusätze sind mit 1.11 entfallen, und der Satz zur Händlerreihenfolge ist gekürzt — aus „This printing’s ISBN was registered in India. Marketplaces that list copies from anywhere come first; no shop was asked.“ wurde „This printing’s ISBN was registered in India, so shops in the German-language area may not carry it. Whether any shop has a copy was not checked.“ Die Regel steht auf der About-Seite; Tatsache, Folge für den Leser und Einschränkung (N12) bleiben. Eine erste Kürzung auf „…; no shop was asked.“ war Julian zu kurz und ohne Zusammenhang. Offen ist nur noch der Verfügbarkeits-Absatz, und der wartet auf 0.1.*
...
*Aus der [Recherche vom 2026-09-12](docs/best-practices-2026-09-12.md) zu Best Practices für Websites und für die Arbeit mit Claude Code, und aus der Bewertung desselben Tages. Keiner dieser Punkte ändert die Seite; jeder verhindert einen Fehler, der schon einmal passiert ist.*

- [ ] **6.39 Hooks für die Regeln, die jedes Mal gelten müssen.** Vier Regeln in CLAUDE.md sind „jedes Mal“-Regeln und wurden trotzdem gebrochen, weil eine Anweisung eine Bitte ist und kein Mechanismus (offizielle Anleitung: „instructions in CLAUDE.md are advisory; hooks are deterministic“). Als Hooks in `.claude/settings.json`, Skripte unter `.claude/hooks/`: (1) `SessionStart` → `npm run worktrees -- --fetch`, damit die Übersicht nicht vergessen werden kann; (2) `PreToolUse` auf `Edit|Write` **blockt** die erzeugten Dateien `docs/worktrees.md`, `docs/kanban.html` und `data/cover-index.json` (Exit-Code 2 mit Begründung); (3) `PostToolUse` auf `Edit|Write` von `ROADMAP.md` → `npm run kanban`, damit das Brett nie eine Stunde alt ist; (4) `PreToolUse` auf `Bash` mit `git push` → erst die Worktree-Übersicht, und ein Push nach `main` fragt nach, weil `origin/main` Produktion ist; (5) ein `Stop`-Hook, der Lint und `npm run test:run` verlangt, wenn in dieser Sitzung Code geändert wurde (Deckel acht Blockaden, `stop_hook_active` beachten). Dazu als Regel, nicht als Hook: **vor jedem Merge nach `main` ein `/code-review` in frischem Kontext.** Eine Stunde, Claude; jeden Hook einmal auslösen und das Ergebnis hier eintragen.

- [ ] **6.40 Ein Skill `/abhaken <Nr>`, und einer für die N14-Messung.** Das Abhaken hat fünf Schritte (Kurzfassung in der Roadmap, Langtext ins Archiv, Zeile in `docs/features.md`, Messung in die Historie, `npm run kanban`), und drei Sitzungen haben Teile ausgelassen: 1.9 behielt zwei offene Kästchen im abgehakten Punkt, 5.8a kam nie in die Roadmap, 6.16 Schritt 1 wurde beim Umbau übersehen. Eine `SKILL.md` unter `.claude/skills/abhaken/` mit den fünf Schritten, der Prüfung „kein `- [ ]` mehr im Block“ und `disable-model-invocation: true`, damit sie bewusst ausgelöst wird. Zweiter Skill `/messen`: die Messung aus 6.30 (abgeschnittene Zeilen, Platzhalter gegen Feld, Seitenbreite) bei 390 und 1280 px als Befehl. Eine Stunde, Claude.
...
- [x] **6.46 Klappentexte auch aus dem Open-Library-Werk-Datensatz.** Erledigt 2026-09-14 als Fallback und als Schalter: `getWorkPage` holt `/works/<id>.json` auf Seite 0 nur, wenn keine Ausgabe einen Klappentext hat (Google aus, Kontingent leer, nichts Passendes) — oder immer, mit `BLURB_SOURCE=work`; `lib/blurb.ts` bereinigt das Markdown und erkennt Wikipedia, das Panel nennt dann Wikipedia (CC BY-SA) statt nur Open Library. Gemessen: 132 von 134 veröffentlichten Werken tragen eine (99 %); live liefert der Fallback für *Gatsby* 994 Zeichen, für *Gravity's Rainbow* nichts und keinen Fehler. Ein Fund beim Testen: Googles zwanzig Gatsby-Bände im Fixture sind fast alle Sekundärliteratur und ergeben **keine einzige** Ausgabe — der Fallback greift dort zu Recht. → [Historie](docs/history.md#2026-09-14--der-klappentext-aus-dem-werk-datensatz-als-fallback-und-als-schalter-roadmap-646) · [Archiv](docs/roadmap-archive.md#646)
...
- [x] **6.35 E-Book-ISBNs fallen weg, das Bild bekommt die Druck-ISBN (E21).** Erledigt 2026-09-11. Gemessen zuerst: **Googles `isEbook` meint nicht den Band** (25 von 100 Bänden, alle gedruckte Bücher mit E-Book-Fassung) und wird nicht benutzt. Eine Ausgabe, die Open Library als E-Book führt, verliert ihre ISBN, dieselbe Nummer auch bei einem Google-Band; das Cover bleibt. Hörbücher warf der Parser schon heraus. Suchlinks nennen Verlag und Jahr nur aus einem gedruckten Open-Library-Datensatz (`searchFacts`) — M10s Suche nach „Hachette UK 2011" wird damit zu Titel und Autor. **Nicht erkannt** bleibt ein E-Book, das kein Katalog als solches führt, wie 9780748130986 selbst. **Offen daran, auf dem Branch `claude/loading-screen-mosaic-animation-72a738` und nicht zusammengeführt (Stand 2026-09-12):** gemessen, wie oft Googles Verlag und Jahr tragen, dazu vier Wege, aus einem Google-Band zu suchen — **welcher davon gilt und wie weit die Regel reicht, entscheidet Julian.** → [Historie](docs/history.md#2026-09-11--karten-ohne-wiederholung-und-nur-gedruckte-bücher-roadmap-634-und-635) · [Archiv](docs/roadmap-archive.md#635)

- [x] **6.37 Doppelte React-Keys auf der Detailseite.** Erledigt 2026-09-11. Ursache gefunden, bevor entdoppelt wurde: `buildWall` hängte die Händler-Cover der ISBN-Nachschau ungeprüft an die Cover der Wand, und auf *Going Postal* war `gb:WkePEAAAQBAJ` beides — Google-Kandidat auf Seite 0 und Googles Bild für ISBN 9780857525086. Die ID stand damit zweimal in der Liste, die gefaltet wird. Jetzt `withRetailCovers` (`lib/works.ts`): ein Cover, das schon da ist, bleibt einmal und bekommt die Ausgaben der Nachschau dazu; das fängt auch zwei ISBNs ab, für die Google denselben Band liefert. Kein Cover fällt weg, das Urteil findet das Händlerbild weiter an seiner ID. Keine doppelte Ausgabe dahinter. → [Historie](docs/history.md#2026-09-11--eine-google-id-zweimal-in-der-liste-der-wand-roadmap-637) · [Archiv](docs/roadmap-archive.md#637)
...
- [ ] **5.8a „Hot or Not" mit Covern — online als Vorschau, mit Speicher.** *Nachgetragen am 2026-09-12: der Punkt wurde am 2026-09-11 in einer eigenen Sitzung gebaut (Branch `worktree-session-2026-09-11`, dreizehn Commits) und stand bis dahin **nirgends in der Roadmap** — nur in den Betreffzeilen.* Aus Spielart 1 von 5.8 geworden: zwei Cover nebeneinander, eine Wahl, die nächsten zwei. Stand laut Commits: hinter einem Schalter auf der Seite, als Vercel-Vorschau mit einem Redis-Speicher, **1.000 Cover aus 200 Büchern** im Topf, erste echte Stimmen sind da; ein kaputtes Bild nimmt das Cover nicht mehr für alle aus dem Spiel; Vorschauen liegen hinter Vercel Authentication, deshalb ein Freigabelink, der eine Bereitstellung lang gilt. **Offen:** der Zweig ist 47 Commits hinter Produktion und nicht zusammengeführt; vor dem Merge gehört geklärt, was davon auf die öffentliche Seite darf (der Speicher ist heute ein Vorschau-Speicher) und wann das Brett ein Cover „das hässlichste" nennen darf — die Regel dazu steht im ersten Commit. **Wer:** Claude baut, Julian entscheidet über das Veröffentlichen.
