...
**Einnahmen jenseits der Affiliate-Links, 2026-09-07** (Julian: „Geld verdienen, ohne komplett seine Ehre aufzugeben“): Analyse und vorläufiger Plan in [docs/plans/PLAN-4-einnahmen.md](docs/plans/PLAN-4-einnahmen.md). Kurzfassung: Affiliate bleibt die Hauptquelle und ist pro Aufruf mindestens so ergiebig wie jede Werbung, die mit N11 (kein Tracking, kein Cookie-Banner) vereinbar ist; Programmatic Display fällt deshalb weg. Was bleibt, ist **ein** stiller, gekennzeichneter Platz ohne Fremdskript, zuerst unbezahlt für Open Library (die Seite lebt von deren Daten, das ist nicht peinlich, weil es wahr ist), später an Direktsponsoren aus Verlag und Buchgestaltung verkauft. Punkte **4.6–4.9**, dazu buch7 in 4.3. Nichts davon vor Phase 2.
...
- [ ] **4.6 Werberegeln in die Spec** (mit Phase 2). Die sechs Sätze aus [PLAN-4-einnahmen.md](docs/plans/PLAN-4-einnahmen.md) Abschnitt 4 als SPEC 2.5 oder E18: ein Platz, kein fremdes Skript, gekennzeichnet, passend, niemand zahlt mit Funktion, der Platz darf leer sein. Dazu der About-Satz. Eine Stunde, Claude; die Entscheidung, dass es überhaupt einen Platz geben soll, trifft Julian davor.
- [ ] **4.7 Der eine Platz** (nach Phase 3). `lib/sponsors.ts` (Text, Bild als Datei im Repo, Ziel, Laufzeit, bezahlt ja/nein), eine Komponente unterhalb der Seitenleiste und in der Fußzeilen-Zone der Startseite, nie in der Schublade; `/go/sponsor/<id>` nach der Regel von F5 (Anbieter, Zeit, keine Kennung); Test, dass ohne Eintrag nichts gerendert wird. Erste Belegung: Open Library, unbezahlt. Ein halber Tag, Claude. **Nicht** der leere Platz oben rechts aus 1.9: dort gehört keine Anzeige hin.
- [ ] **4.8 Direktsponsoren und Kontextwerbung** (ab etwa 10.000 Detailseiten-Aufrufen im Monat, Zahl aus 3.1). Mediakit aus Aggregaten (Search Console, Vercel Analytics), zehn Kandidaten aus Verlag, Buchgestaltung, Buchbinderei, Literaturzeitschriften ansprechen, Festpreis pro Monat statt CPM. Carbon Ads oder EthicalAds als Fallback anfragen, nur wenn sie ohne Cookie und Kennung liefern (PLAN-4-einnahmen, Frage 6.4). Julian. Vorher die offenen Fragen 6.2 (Kennzeichnungsrecht) und 6.3 (Google-Books-Bedingungen im Wortlaut) klären.
- [ ] **4.9 Den Platz messen** (drei Monate nach 4.7). Klicks je Belegung; unter 0,1 % Klickrate den Platz überdenken, nicht die Regeln. Ein Spendenknopf für die Seite selbst (Ko-fi, Liberapay) kann im selben Zeitraum als eine Fußzeilen-Zeile mitlaufen und wird nach drei Monaten behalten oder gestrichen. **Programmatic Display, Bezahlfunktionen mit Google-Daten und Merch aus Cover-Wänden bleiben ausgeschlossen**, solange N11, die Google-Bedingungen und die Bildrechte gelten; die Begründung steht im Plan.
