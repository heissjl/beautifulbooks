...
**Julians Entscheidung dazu (2026-09-11): E-Book-ISBNs werden unterdrückt, es geht nur um physische Bücher.** Festgehalten als [SPEC E21](../../SPEC.md#6-entscheidungen). In den aufgenommenen Fixtures (fünf Werke) tragen 25 von 100 Google-Bänden `isEbook: true`, und Open Library führt 40 Ausgaben als „Epub"/„ebook" — dazu 12 Hörbuch-Formate, die nach Julians zweiter Entscheidung desselben Tages ebenso fallen („die sollten wir ebenfalls wie eBooks behandeln").
...
## Tranche 4 — Die alte Wand, und eine Idee

### M11 Die erste pulsierende Wand taucht noch auf

Julian: *„Manchmal wird noch der allererste Loading-Screen mit der pulsierenden Wand angezeigt. Der sollte nirgendwo mehr existieren."*

Im Code gefunden: das ist `AssemblingWall`, die kleine Wand aus den Startseiten-Covern. `MosaicLoader` zeigt sie **solange die Mosaik-Datei noch nicht decodiert ist** und dauerhaft, wenn sie nicht kommt (`if (failed || !scene) return <AssemblingWall …/>`). Vorgeladen wird die Datei nur, wenn jemand ins Suchfeld tippt; auf jedem anderen Weg — Werkseite von außen, Jahrzehnte-Seite — steht die alte Wand für die Dauer eines Abrufs von rund 90 KB da. Weitere pulsierende Flächen, von denen offen ist, ob Julian sie mitmeint: die Titelbalken auf Werkseite und Jahrzehnte-Ladeseite, und das Karten-Cover in `LoadingStage`, das pulsiert, bevor der Fächer beginnt.

### M12 Idee fürs Lab: Gebrauchsspuren

![Gebrauchsspuren](2026-09-11-m12-gebrauchsspuren.jpg)

Julian, an einem Open-Library-Scan des *Infinite Jest* mit Knicken, weißen Kanten und eingerissener Ecke: *„Diesen Effekt auf andere Cover legen können → neues Lab-Projekt."* Aufgenommen als ROADMAP **5.9** und als Zeile `wear/` in `lab/README.md`.

---

## Plan

Die Befunde sind in [ROADMAP.md](../../ROADMAP.md) als Punkte angelegt, die Reihenfolge steht unter „Nächste Schritte". Die Regel, die aus Julians Anforderung folgt, ist [SPEC N14](../../SPEC.md#4-nicht-funktionale-anforderungen); die Entscheidung zu E-Books ist [E21](../../SPEC.md#6-entscheidungen).

| Befund | Punkt | Telefon-spezifisch? |
|---|---|---|
| M1 Platzhalter, M2 Titel, M3 Leerraum | **6.30** | ja |
| M4 Karten-Dubletten, M5 Größe L | **6.34** | M5 trifft das Telefon härter |
| M6 Wand übernimmt leer | **6.25a**, gebaut, nicht deployt | nein |
| M7 Kacheln bleiben leer | **6.25** (Ursache, braucht die Logs), **6.31** (zweiter Versuch) | vermutlich nein |
| M8 falsches Urteil | **6.32**, dazu 6.25 und 6.36 | nein |
| M9 Wand-Dubletten | 6.25 (fehlende Signaturen), **6.36** (Julians Entscheidung) | nein |
| M10 E-Book-ISBN | **6.35**, E21 | nein |
| M11 alte Wand | **6.33** | nein |
| M12 Gebrauchsspuren | **5.9**, `lab/wear/` | — |

**Das Ergebnis zur Anforderung „vergleichbare Erfahrung":** von elf Befunden sind drei telefon-spezifisch (M1–M3), und alle drei haben dieselbe Ursache — Maße, die am Desktop gewählt wurden. Die übrigen treffen beide Geräte; das Telefon fiel nur auf, weil dort getestet wurde. Deshalb ist die Antwort eine Prüfregel (N14, und in CLAUDE.md), nicht ein Umbau.
