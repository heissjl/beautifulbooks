...
| Ein Rondell aus sieben Covern eines kuratierten Buchs, bei jedem Besuch ein anderes (90 von 105), rechts neben Überschrift und Suchfeld, ab `lg`, als Link auf die Wand; dreht sich selbst und folgt der Maus, vorne und hinten entgegengesetzt; die Ringe beim Bauen nach Abstand gewählt, jedes Paar über der lockersten Hash-Schwelle der Seite, testgeprüft; unter `lg` gar nicht gerendert | 2026-09-10; Rondell und Wechsel 2026-09-11 | F6 | 1.9 | `components/HeroFan.tsx`, `components/HeroRondell.tsx`, `lib/herofan.ts`, `lib/heroring.ts`, `scripts/build-hero-rings.ts` |
...
| Faltung gleicher Scans im Browser, vier Stufen (≤ 8; ≤ 20 bei gleicher ISBN; ≤ 16 bei gleichem Verlag ± 1 Jahr; ≤ 13 über Verlagsgrenzen, wenn beide Bilder einen Entwurf tragen und dieselbe Farbwelt haben); nie über Sprachen | 2026-09-07 / 09-23 | §2.3, E8 | Schritt 12 | `lib/works.ts` (`foldDuplicateCovers`), `lib/imagesig.ts`, `lib/imagehash.ts` (Server) |
...
| Der Klappentext hat einen Fallback und einen Schalter: fehlt jeder Ausgaben-Klappentext (Google aus, Kontingent leer), holt der Server die Werk-Beschreibung von Open Library — bereinigt von Markdown, Wikipedia als Quelle genannt, wo der Text sie nennt; `BLURB_SOURCE=work` zieht sie immer vor | 2026-09-14 | F2.6a | 6.46 | `lib/blurb.ts`, `lib/sources/openlibrary.ts` (`getWorkDescription`), `lib/work.ts`, `lib/works.ts` (`blurbFor`), `components/WorkPanel.tsx` |
...
| „N scans of this cover“: jeder gefaltete Scan ist anklickbar und tauscht das große Bild; Quelle des gezeigten Scans wird genannt; eine Reihe, die seitwärts scrollt, mit Verlaufskante nur solange es weitergeht | 2026-09-09 / 09-10 | §2.3 | 6.14, 6.14a | `components/BookDetail.tsx` (`CoverDetails`), `useOverflowsX.ts` |
...
| Thematische Sammlungen `/collections`: Wände von Hand gewählter oder aus einer Reihe gebauter Cover; veröffentlicht: SF Masterworks 1–73; auf der Startseite unten bis zu vier Karten mit der ersten Reihe | 2026-09-25 | F8 | 5.10, 5.10c | `app/collections/`, `lib/collections.ts`, `data/collections.json` |
...
| Sammlungs-App: Sammlungen anlegen (Autorinnen oder Verlagsreihe), Grenze pflegen, Kandidaten je Autorin, Cover wählen, Reihenfolge ziehen; Entwürfe und Vorschläge von Freunden übernehmen oder ablehnen | 5.10, 5.10a, 5.10b | `lab/collections/` |
| Kuratier-App online für Freunde (`/curate`, hinter Passwort): Sammlungen als Entwürfe bauen, die Julian übernimmt; dazu `/suggest` für ein einzelnes Buch | 5.10a, 5.10b | `app/curate/`, `components/CurateTool.tsx`, `lib/curate/` |
| Duell-Prototyp: zwei Menschen, ein Link, dieselbe Runde | 5.8 | `lab/duel/` |
| Hot or Not für Cover: zwei Cover, ein Klick, Rangliste mit Unsicherheit und einer Krone erst nach drei gehaltenen Runden; Simulation dazu. Auf der Seite als `/versus` hinter Schalter, von der Startseite aus verlinkt, seit 2026-09-23 indexiert (mit Text und 24 Buchlinks für Crawler), **in Produktion seit 2026-09-22** mit demselben Speicher wie die Preview (SPEC F7). 2000 Cover aus 499 Büchern (seit 2026-09-26), nur solche, die groß scharf bleiben, nicht überwiegend weiß, kein Seiten-Scan und kein klassisches Reclam-Heft sind, und was neu hinzukam, auch scharf gescannt und einzeln angesehen; Titel und Autor unter jedem Cover; kein Cover kehrt innerhalb von 50 Paaren wieder; nach dem Pick ein Stempel, die nächsten zwei Paare sind schon vorgeladen, und ein Link zur Buchseite mit genau diesem Cover (auch von der Rangliste); unter jedem Cover ein „Share"-Knopf und ein Link „All covers" auf die ganze Wand des Buchs (neuer Tab); die Rangliste zeigt fünf oder, auf Knopfdruck, zwanzig je Ende; jedes siebte Paar stellt zwei Ausgaben desselben Buchs gegeneinander, nie zweimal dieselbe Jacke | 5.8, 5.8a | `lab/hotornot/`, `lib/hotornot/`, `app/versus/` |
