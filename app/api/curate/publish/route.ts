 ...
 * drafts published from /curate (ROADMAP 6.54, the Cockpit's view of the
...
export async function GET(request: NextRequest) {
  ...
  try {
    const [switches, content] = await Promise.all([store.get(), store.getContent()]);
    return json({ switches, content });
  } catch {
