...
interface EditionsByLanguageProps {
  ...
  groups: LanguageTab[];
  selectedEdition: EditionView | null;
  onSelectEdition: (edition: EditionView) => void;
}

const tabKey = (g: LanguageTab) => g.language ?? 'unknown';

export default function EditionsByLanguage({ groups, selectedEdition, onSelectEdition }: EditionsByLanguageProps) {
  // The tab follows the selected edition unless the user picked a tab
  // since the selection last changed (derived state, no effect needed).
  const [picked, setPicked] = useState<{ key: string; forSelectedId: string | null } | null>(null);
  const selectedId = selectedEdition?.id ?? null;
  const ownerKey = selectedEdition
    ? groups.find(g => g.editions.some(e => e.id === selectedEdition.id))
    : undefined;
  const activeKey =
    picked && picked.forSelectedId === selectedId
      ? picked.key
      : ownerKey ? tabKey(ownerKey) : groups[0] ? tabKey(groups[0]) : 'unknown';
  const setActiveKey = (key: string) => setPicked({ key, forSelectedId: selectedId });
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg border border-amber-100 overflow-hidden p-6 sm:p-8">
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-900 mb-2">All Editions &amp; Covers</h2>
        <p className="text-gray-600 text-sm">
          {total} edition{total !== 1 ? 's' : ''} with covers across {groups.length} language{groups.length !== 1 ? 's' : ''}
        </p>
      </div>
          ...
          <div className="flex flex-wrap gap-2 -mb-px">
            {groups.map(g => (
              ...
              >
                ...
                <span className="ml-2 text-xs bg-gray-100 px-2 py-0.5 rounded-full">{g.editions.length}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-8 gap-3 sm:gap-4">
        {active?.editions.map(edition => {
          ...
          return (
            ...
            >
              <Image
                src={edition.coverUrlSmall ?? edition.coverUrl}
                alt={`${edition.title}${edition.publisher ? `, ${edition.publisher}` : ''}${edition.year ? ` ${edition.year}` : ''}`}
                fill
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-2">
                <div className="text-white text-xs text-left">
                  {edition.year && <p className="font-semibold">{edition.year}</p>}
                  {edition.publisher && <p className="line-clamp-1">{edition.publisher}</p>}
                </div>
              </div>
              {selected && (
